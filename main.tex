...
\begin{document}
	...
	\begin{example}Demonstre ângulos notáveis ($30^\circ, \, 45^\circ, \, 60^\circ$).
		...
		Para isso, consideramos um triângulo equilátero de lado \(2l\), com todos os ângulos internos iguais a \(60^\circ\). Ao traçarmos uma altura a partir de um vértice até o lado oposto, dividimos o triângulo em dois triângulos retângulos.
...
\renewcommand{\arraystretch}{1}
	...
	Considere, novamente, os dois gráficos da Fig. abaixo.
		...
		\begin{subfigure}[t]{0.48\textwidth}
			...
			\begin{tikzpicture}
				...
				\end{axis}
			\end{tikzpicture}
			\caption{Gráfico da função descontínua, com $p \in D_f$}
			\label{fig:b2}
		\end{subfigure}
	...
	Observe na Fig. (a) que após o ponto $p$, $x$ não possui uma imagem associada na vizinhança do ponto $f(p)$. Sabemos que essa função é descontínua, pois apresenta um salto no gráfico.
	
	Esse salto é mais sutil no gráfico da Fig. (b). Perceba que existe uma descontinuidade no ponto $(p,h(p))$, ainda que $p \in D_f$.
	...
	\begin{snugshade}
		...
		Na Fig.~\ref{fig:comparacao-funcao-continua2} (b), $p \in D_f$. Contudo, a função é descontínua em $p$. Em outras palavras, $h(p)$ \textbf{está} definida para $p$, ainda que o valor esperado não seja o mesmo.
