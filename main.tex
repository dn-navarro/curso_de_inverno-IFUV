...
\begin{document}
	...
	\subsubsection{Uma comparação visual entre a função $f(x) = x^2-6x+8$ e $|(x-2)(x-4)|$.}
...
\renewcommand{\arraystretch}{1}
		...
		\begin{subfigure}[t]{0.48\textwidth}
			...
			\begin{tikzpicture}
				\begin{axis}[
					...
					\addplot[domain=2.5:4, smooth, thick, blue] {-ln(x)*cos(deg(x))};
					...
					\addplot+[only marks, mark=o, mark options={draw=blue}, blue, thick] coordinates {(2.5, 0.7340805)};
	...
	\begin{align*}
		...
		\textbf{h)}\quad & \lim_{x \to -3} \sqrt[3]{x} \approx -1,442 \quad\text{(função contínua em } \mathbb{R}) \\
	...
	\begin{definition}[Derivada de uma função]\label{def:derivada}
		...
		\begin{equation}
			\boxed{f'(p) = \lim_{h \to 0} f(p) = \dfrac{f(p - h) - f(p)}{h}} 
		\end{equation}
	...
	\subsubsection{A equação da reta}
	\begin{snugshade}
		\textbf{\textcolor{blue}{Relembre!}}
