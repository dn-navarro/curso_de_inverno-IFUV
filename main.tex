...
\begin{document}
	...
	Matemática, num primeiro momento, é linguagem! Isso significa que nos comunicamos com as outras pessoas e expressamos ideias matemáticas por meio de símbolos.
	...
	\begin{snugshade}
		...
		\begin{center}
			\begin{tikzpicture}[scale=2]
				...
				% Título
				\node at (3,3.3) {\textbf{Conjuntos Numéricos}};
		...
		\begin{itemize}
			%%% Colocar um comentário explicando que para alguns matemáticos o zero não pertence aos reais.
			\item[] \textbf{Conjunto dos Naturais:} $\mathbb{N} = \{0, 1, 2, 3, 4, 5, \dots\}$. Não há consenso na comunidade sobre o pertencimento do zero no conjunto dos números naturais. Neste material, você irá reparar que o incluímos no conjunto.
			\item[] \textbf{Conjunto dos Inteiros:} $\mathbb{Z} = \{\dots, -3, -2, -1, 0, 1, 2, 3, \dots\}$
			\item[] \textbf{Conjunto dos Racionais:} $\displaystyle \mathbb{Q} = \left\{ \dfrac{a}{b} \,\middle|\, a \in \mathbb{Z},\, b \in \mathbb{Z}^* \right\}$. Lê-se ``\textit{o conjunto de todas as frações $a$ sobre $b$, tal que $a$ pertence ao conjunto dos inteiros, e $b$ pertence ao conjunto dos inteiros diferentes de zero.}''
			
			Exemplos: $\mathbb{Q} = \left\{ -1,\, \dfrac{1}{2},\, 0,\, 3,\, \dfrac{22}{7},\, \dots \right\}$
			
			\begin{example}
				Considere $\displaystyle \dfrac{\sqrt{2}}{2}$. Ele é um número racional? Não, pois $\sqrt{2} \notin \mathbb{Z}$ (lê-se \textit{``raiz de dois não pertence ao conjunto dos números inteiros''}).
				
				E um número qualquer $\displaystyle \dfrac{a}{b}$, com $b = 0$? Também não, essa fração não está definida.
			\end{example}
			
			\item[] \textbf{Conjunto dos Irracionais:} $\mathbb{R} - \mathbb{Q} = \{\pi,\, \sqrt{2},\, e,\, \dots\}$. O sinal ($-$), nesse caso, é um operador de diferença entre conjuntos, e lemos \textit{``o conjunto dos números irracionais é o conjunto dos números reais, menos o conjunto dos números racionais''}.
		\end{itemize}
	\end{snugshade}
	...
	\begin{snugshade}
		...
		Considere dois números racionais \( \dfrac{a}{b} \) e \( \dfrac{c}{d} \), com \( b \neq 0 \) e \( d \neq 0 \).  
		...
		\[
		\dfrac{\dfrac{a}{b}}{\dfrac{c}{d}} \times \textcolor{red}{1}
		\]
	...
	\begin{align*}
		\textcolor{red}{1 = \dfrac{\dfrac{d}{c}}{\dfrac{d}{c}}}, \quad \text{com } c \neq 0
	\end{align*}
		
		
		
		Isto é verdade porque estamos efetuando a divisão de um número por ele mesmo e, além disso, observe que \( \dfrac{d}{c} \) é o inverso de \( \dfrac{c}{d} \). Voltando ao problema inicial:
		
		
		
		\begin{align*}
			\dfrac{\dfrac{a}{b}}{\dfrac{c}{d}} &= \dfrac{\dfrac{a}{b}}{\dfrac{c}{d}} \times \textcolor{red}{1}\\
			&= \dfrac{\dfrac{a}{b}}{\dfrac{c}{d}} \times \textcolor{red}{\dfrac{\dfrac{d}{c}}{\dfrac{d}{c}}} \\
			&= \dfrac{\dfrac{a\cdot d}{b \cdot c}}{\dfrac{\cancel{c} \cdot \cancel{d}}{\cancel{d} \cdot \cancel{c}}}\\
			&= \dfrac{\dfrac{a\cdot d}{b \cdot c}}{1}\\
	...
	\begin{example}Operações com frações
		\begin{itemize}
			\item[a)] $\displaystyle\dfrac{1}{2} + \dfrac{3}{5} =$
			
			\begin{align*}
				\dfrac{1}{2} + \dfrac{3}{5} &= {\textcolor{gray}{\dfrac{5}{5}}}\cdot\dfrac{1}{2} + \dfrac{3}{5} \cdot {\textcolor{gray}{\dfrac{2}{2}}} \quad \text{(uma multiplicação esperta: $\dfrac{5}{5} = \dfrac{2}{2} = 1$)}\\
				&= \dfrac{5}{10} + \dfrac{6}{10} \quad \text{(os denominadores foram igualados)}\\
				&= \dfrac{11}{10}
			\end{align*}
			
			\item[b)] $\displaystyle\dfrac{7}{5} \div \dfrac{3}{4} =$
			
			\begin{align*}
				\dfrac{7}{5} \div \dfrac{3}{4} &= \dfrac{\dfrac{7}{5}}{\dfrac{3}{4}} \quad \text{(Dividir por fração é multiplicar pelo inverso)}\\
				&= \dfrac{7}{5} \cdot \dfrac{4}{3}\\
				&= \dfrac{28}{15}
			\end{align*}
		\end{itemize}
	\end{example}
	...
	\subsubsection{Reconhecendo funções a partir de exemplos.}
	...
	\subsubsection{Reconhecendo o domínio, contradomínio e imagem de funções.}
	...
	\begin{snugshade}
		...
		\begin{center}
			$Im_f = \{b \in B \,|\, b = f(a), a \in D_f\}$
			
			(Lê-se ``um elemento $b$ pertence ao conjunto $B$, tal que $b$ é a função $f$ calculada em um $a$, e $a$ pertence ao $D_f$'')
		\end{center}
		...
		\begin{align*}
			D_f &= \mathbb{R}\\
			Im_f &= \{y \in \mathbb{R} \,|\, y = f(x), x \in D_f\}\\
	...
	\subsubsection{Uma função especial: a função constante.}
	...
	\subsubsection{O plano cartesiano}
	...
	Os pares ordenados $(2,3)$ e $(3,2)$, indicados acima, são diferentes.
	
	Vejamos, agora, outro exemplo.
	
	%%Rever a sequência: o exemplo deve vir logo antes do plano cartesiano. Retome o exemplo e explique a notação do par ordenado G_f
	...
	\textbf{Analisando gráficos e determinando se são funções}
	
	Planos cartesianos também são úteis para analisarmos se uma curva representa uma função.
	
	Conforme vimos anteriormente, para que uma curva represente uma função ela deve atender ao critério da \textit{unicidade}, ou seja, cada ponto $x$ deve ter apenas uma correspondência em $y$. Observe:

		
		\begin{figure}[h!]
			\centering
			
			% Gráfico que representa uma função
			\begin{subfigure}[t]{0.45\textwidth}
				...
				\begin{tikzpicture}
					...
					\end{axis}
				\end{tikzpicture}
				\caption{Cada valor de $x$ tem um único valor de $y$.}
			\end{subfigure}
			\hfill
			% Gráfico que NÃO representa uma função
			\begin{subfigure}[t]{0.45\textwidth}
				\centering
				\begin{tikzpicture}
					% Eixos
				...
				\end{tikzpicture}
				\caption{Alguns valores de $x$ têm dois valores de $y$.}
			\end{subfigure}
			
			\caption{Visualização do critério da unicidade: à esquerda, uma função; à direita, uma curva que não é função.}
			\label{fig:unicidade_dashed}
		\end{figure}
	...
	\begin{practice}
		...
		\begin{enumerate}
			...
			\begin{align*}
				...
				f(1) &= 1^2 = 1 \\
				\\
				\dfrac{f(x) - f(1)}{x - 1} &= \dfrac{x^2 - 1}{x - 1} \\
				&= \dfrac{(x - 1)(x + 1)}{x - 1} \\
				&= x + 1 \quad \text{(para } x \ne 1\text{)}
			\end{align*}
	...
	\begin{tabular}{cc}
		...
		Quando $y = 0$, & $x = -\displaystyle\dfrac{1}{2}$ (resolvemos $f(x) = 2x + 1 = 0$)
	\end{tabular}
	...
	\begin{align*}
		...
		= x &= -\dfrac{1}{2}
	\end{align*}
	Isso significa que a reta cruzará o eixo $x$ quando $x= \displaystyle -\dfrac{1}{2}$.
	
	\subsubsection{Analisando o coeficiente angular.}\label{tema:coef_angular}
	...
	Vamos ampliar o triângulo e analisá-lo na Fig.~\ref{fig:funcao_afim_3}. Note que a base tem tamanho igual a $\displaystyle \dfrac{1}{2}$, e formamos um ângulo que chamamos de $\theta$ (lê-se \textit{téta}).
	...
	\begin{figure}[h]
		...
		\begin{tikzpicture}[scale=3]
			...
			\path (A) -- (B) node[midway, below] {$\dfrac{1}{2}$};
	...
	\begin{align*}
		...
		\tan{\theta} &= \dfrac{1}{\displaystyle\dfrac{1}{2}} \text{ \textit{(dividir por fração é o mesmo que multiplicar pelo inverso)}}\\
		\tan{\theta} &= 1 \cdot \dfrac{2}{1}\\
		\tan{\theta} &= 2\\
	\end{align*}
		...
		\begin{itemize}
			\item Fórmula quadrática (o popular Bhaskara):
				...
				\begin{align*}
					...
					x_{1,2} &= \dfrac{- b \pm \sqrt{\Delta}}{2 \cdot a}
				\end{align*}
				
				Também podemos calcular de uma maneira mais direta:
				
				\begin{align*}
					x_{1,2} &= \dfrac{- b \pm \sqrt{b^2 - 4ac}}{2 \cdot a}
				\end{align*}
	...
	\subsubsection{Analisando o comportamento da função}
	...
	\begin{example}\label{ex:f_x2-6x+8}
		...
		\begin{align*}
			f(x) &= x^2 -6x + 8 \\
			0 &= x^2 -6x + 8 \\
			x &= \dfrac{-(-6) \pm \sqrt{(-6)^2 - 4 \cdot 1 \cdot 8}}{2 \cdot 1} \quad &\text{(Fórmula quadrática)}\\
		...
		\begin{align*}
			x_1 &= \dfrac{6 + 2}{2} = \dfrac{8}{2} = 4 \\
			x_2 &= \dfrac{6 - 2}{2} = \dfrac{4}{2} = 2
		\end{align*}
	...
	\subsubsection{Analisando o discriminante ($\Delta$)}
	
	Uma função quadrática sempre cortará o eixo $x$? Não! Observe o exemplo a seguir:
	...
	\subsubsection{Analisando a concavidade ($a > 0$, $a < 0$).}
	...
	O termo dominante é aquele que multiplica a variável independente de maior expoente.
	...
	\begin{example}
		Seja a função $f(x) = \displaystyle \dfrac{x^2 - 2x - 1}{2x-1}$. Recorde-se de que $2x-1 \neq 0$. Assim, temos
	...
	\begin{example} A função a seguir \textbf{não} é uma função racional, já que o numerador não é um polinômio. Observe que o expoente $\displaystyle\dfrac{1}{2} \notin \mathbb{N}$ (lê-se \textit{meio não pertence ao conjunto dos números naturais}).
		
		\begin{equation*}
			f(x) = \dfrac{\sqrt{x-1}}{x^2+1} = \dfrac{(x-1)^{\dfrac{1}{2}}}{x^2+1}
		\end{equation*}
	\end{example}
	...
	\begin{snugshade}
		...
		\begin{itemize}
			...
			\item $4^\dfrac{1}{2} = \sqrt{4} = 2$ 
			\item $\displaystyle 2^{-1} = \dfrac{1}{2^1} = \dfrac{1}{2}$
			\item $\displaystyle 0^{-1} = \dfrac{1}{0^1} = \dfrac{1}{0}$ (Opa! Não é possível realizar a divisão por zero)
			\item $1^x = 1, \, x \in \mathbb{R}$ (O resultado é sempre igual a $1$)
		\end{itemize}
		
		
		Qualquer número elevado a uma fração pode ser escrito como uma raiz:
		\begin{equation*}
			b^{\dfrac{{\textcolor{blue}{n}}}{{\textcolor{red}{d}}}} = \sqrt[{\textcolor{red}{d}}]{b^{\textcolor{blue}{n}}}
		\end{equation*}
		...
		Quando $b<0$, a função não está definida para todo $\dfrac{n}{d} \in \mathbb{R}$. Em outras palavras, quando $b<0$, o gráfico dessa função é cheio de buracos.
	\end{snugshade}
	...
	\begin{practice}
		...
		\begin{enumerate}
			...
			\item[o)] $f(x) = \displaystyle \dfrac{x^2 - 1}{x - 1}$
			
			\begin{align*}
				f(x) &= \dfrac{x^2 - 1}{x - 1} \quad \text{(Repare a diferença de quadrados no numerador)}\\
				&= \dfrac{\cancel{(x - 1)}(x + 1)}{\cancel{(x - 1)}} \\
				&= x + 1,\quad \text{com } x \neq 1 \\
				\text{Domínio: } & \mathbb{R} - \{1\} \\
			\end{align*}
			...
			\begin{figure}[h]
				...
				\begin{tikzpicture}
					\begin{axis}[
						...
						\addlegendentry{$f(x) = \dfrac{x^2 - 1}{x - 1}$}
	...
	\begin{practice}
		...
		\begin{align*}
			...
			\text{Para que } y \in \mathbb{R},\quad &\dfrac{x - 1}{x + 1} \geq 0 \\
			\text{Iremos estudar o sinal da expressão:} \quad &\dfrac{x - 1}{x + 1} \geq 0 \\
		...
		Nas duas primeiras linhas, analisamos cada parte da expressão, individualmente, nos pontos onde encontramos as restrições ($-1$ e $1$). Na última linha, analisamos o sinal da expressão toda, usando a regra de sinal em cada região delimitada. Lembre-se: queremos o(s) intervalo(s) onde $x \geq 0$, ou seja, onde a reta não for negativa.
	...
	\subsection{Módulo}\label{subsec:modulo}
	...
	\begin{definition}[Módulo de um número]
		\begin{align*}
	...
	\begin{example}Elimine $|x - 1|$. A partir da definição, temos
	...
	\subsubsection{Uma comparação visual entre a função $f(x) = x^2-6x+8$ e $g(x) = |(x-2)(x-4)|$.}
	...
	\subsection{Funções compostas}\label{subsec:funcoes_compostas}
	...
	\subsubsection{Analisando o domínio das funções compostas.}
	...
	\subsubsection{A distância entre dois pontos no plano cartesiano.}
	...
	\subsubsection{Calculando a inclinação da reta}
	...
	\begin{proof}[A equação da reta.]
		\begin{align*}
			\tan{\theta} &= \dfrac{\text{cateto oposto}}{\text{cateto adjacente}}\\
			\tan{\theta} &= \dfrac{y - y_0}{x - x_0}\\
			\tan{\theta} &= \dfrac{y - y_0}{x - x_0}=m \quad \text{(onde $m$ é o coeficiente angular)}
		\end{align*}
	...
	\subsubsection{A distância entre qualquer ponto no plano e o centro de uma circunferência.}
	...
	\begin{proof}[A distância entre o centro e um ponto $P$ qualquer ]\label{proof:circunferencia}
		\begin{align*}
	...
	\begin{example} Seja $C = (1,2)$ o centro de uma circunferência de raio $R = 2$.
		...
		\begin{align*}
			R^2 &= (x - 1)^2 +(y - y_1)^2 \\
			2^2 &= (x - 1)^2 +(y - 2)^2 \quad \text{(Substitua $x_1=1$ e $y_1 = 2$)}\\
			4 &= (x - 1)^2 +(y - 2)^2
		\end{align*}
	\end{example}
	...
	\subsubsection{Quantos radianos há em uma circunferência?}
	...
	Ou seja, em metade de uma circunferência, equivalente a 180º graus, há $\pi\, rad$. Em uma circunferência completa, equivalente a 360º, há $2 \pi\, rad$. Daí, podemos afirmar que:
	
	\begin{equation}\label{eq:converter_angulo}
	...
	\subsubsection{Conversão de ângulos}
	
	A partir da Equação~\ref{eq:converter_angulo} podemos converter qualquer ângulo em radianos. A seguir, algumas conversões de ângulos úteis:
	
	\renewcommand{\arraystretch}{2}
	\begin{table}[h]
		\centering
		\caption{Conversão de ângulos: de graus para radianos}
		\begin{tabular}{|c|c|c|}
			\hline
			\textbf{Ângulo (graus)} & \textbf{Cálculo} & \textbf{Ângulo (radianos)} \\
			\hline
			\( 30^\circ \)  & \( \dfrac{30 \cdot \pi}{180} \)     & \( \dfrac{\pi}{6} \) \\
			\hline
			\( 45^\circ \)  & \( \dfrac{45 \cdot \pi}{180} \)     & \( \dfrac{\pi}{4} \) \\
			\hline
			\( 60^\circ \)  & \( \dfrac{60 \cdot \pi}{180} \)     & \( \dfrac{\pi}{3} \) \\
			\hline
			\( 90^\circ \)  & \( \dfrac{90 \cdot \pi}{180} \)     & \( \dfrac{\pi}{2} \) \\
			\hline
			\( 120^\circ \)  & \( \dfrac{120 \cdot \pi}{180} \)     & \( \dfrac{2 \pi}{3} \) \\
			\hline
			\( 180^\circ \) & \( \dfrac{180 \cdot \pi}{180} \)    & \( \pi \) \\
			\hline
			\( 210^\circ \) & \( \dfrac{210 \cdot \pi}{180} \)    & \( \dfrac{7\pi}{6} \) \\
			\hline
			\( 240^\circ \) & \( \dfrac{240 \cdot \pi}{180} \)    & \( \dfrac{4\pi}{3} \) \\
			\hline
			\( 300^\circ \) & \( \dfrac{300 \cdot \pi}{180} \)    & \( \dfrac{5\pi}{3} \) \\
			\hline
			\( 330^\circ \) & \( \dfrac{330 \cdot \pi}{180} \)    & \( \dfrac{11\pi}{6} \) \\
			\hline
			\( 360^\circ \) & \( \dfrac{360 \cdot \pi}{180} \)    & \( 2\pi \) \\
			\hline
		\end{tabular}
	\end{table}
	\renewcommand{\arraystretch}{1}
	
	\textbf{Trigonometria do triângulo retângulo}
	...
	\begin{equation}
		\boxed{\sin{\theta} = \dfrac{\text{cateto oposto}}{\text{hipotenusa}} = \dfrac{a}{c}}
	\end{equation}
	
	\begin{equation}
		\boxed{\cos{\theta} = \dfrac{\text{cateto adjacente}}{\text{hipotenusa}} = \dfrac{b}{c}}
	\end{equation}
	
	\begin{equation}
		\boxed{\tan{\theta} = \dfrac{\text{cateto oposto}}{\text{cateto adjacente}} = \dfrac{a}{b}}
	\end{equation}
	
	\begin{proof}[Mostrando que $\tan{\theta} = \displaystyle \dfrac{\sin{\theta}}{\cos{\theta}}$]
		
		\begin{align*}
			\dfrac{\sin{\theta}}{\cos{\theta}} &= \displaystyle \dfrac{\displaystyle \dfrac{a}{c}}{\displaystyle \dfrac{b}{c}}\\
			&= \displaystyle \dfrac{a}{c} \cdot \dfrac{c}{b} \quad \text{(Dividir por fração é multiplicar pelo inverso)}\\
			&= \displaystyle \dfrac{a}{{\cancel{c}}} \cdot \dfrac{{\cancel{c}}}{b} \quad \text{(Simplifique $\dfrac{c}{c} = 1$)}\\
			&= \dfrac{a}{b} = \tan{\theta} \qedhere
		\end{align*}
	\end{proof}
	...
	\begin{example}Demonstre ângulos notáveis ($30^\circ, \, 45^\circ, \, 60^\circ$).
		...
		\begin{align*}
			\theta = \dfrac{180 - 90}{2} = 45^\circ = \dfrac{\pi}{4}
		\end{align*}
		...
		\begin{align*}
			\tan{(\theta)} = \tan{\left(\dfrac{\pi}{4}\right)} = \dfrac{l}{l} = 1
		\end{align*}
		...
		\begin{align*}
			...
			h &= l\sqrt{2}	\quad \text{(Consideramos o valor positivo, já que se trata de distância)}
		\end{align*}
		
		E agora podemos encontrar
		
		\begin{align*}
			\sin{\theta} = \sin{\left(\dfrac{\pi}{4}\right)} &= \dfrac{l}{l\sqrt{2}}\\
			&= \dfrac{l}{l\sqrt{2}} \cdot \dfrac{\sqrt{2}}{\sqrt{2}} \quad \text{(Racionalizamos)}\\
			&= \dfrac{\cancel{l}}{\cancel{l}\sqrt{2}} \cdot \dfrac{\sqrt{2}}{\sqrt{2}} \quad \text{($\sqrt{2} \cdot \sqrt{2} = \sqrt{4} = 2$)}\\
			&= \dfrac{\sqrt{2}}{2}
		\end{align*}
		
		Observe que o mesmo serve para $\displaystyle \cos{\left(\dfrac{\pi}{4}\right)} = \dfrac{l}{l\sqrt{2}} = \dfrac{\sqrt{2}}{2}$.
		
		\textbf{Para os ângulos \(30^\circ\) e \(60^\circ\).}
		
		Para isso, consideramos um triângulo equilátero de lado \(l\), com todos os ângulos internos iguais a \(60^\circ\). Ao traçarmos uma altura a partir de um vértice até o lado oposto, dividimos o triângulo em dois triângulos retângulos.
	
		
		\begin{center}\label{fig:triangulo_equilatero}
			\begin{tikzpicture}[scale=1.4]
				% Pontos
				\coordinate (A) at (0,0);   % Base esquerda
				\coordinate (B) at (4,0);   % Base direita
				\coordinate (C) at (2,3.464); % Topo (altura = 2*sqrt(3))
				...
				% Altura
				\draw[dashed, red] (C) -- (D);
				\node[left, red] at (2, 1.732) {\small $\dfrac{l\sqrt{3}}{2}$};
				
				% Marcação dos lados
				\node[blue] at (2,-0.35) {\small $l$}; % base
				\node[blue] at (3.2,1.7) {\small $l$}; % lado
				\node[blue] at (0.8,1.7) {\small $l$}; % lado
				
				% Marcação do ângulo de 60°
				\draw (B) ++(-0.5,0) arc[start angle=180,end angle=120,radius=0.5];
				\node at (3.4,0.4) {\small $30^\circ$};
				
				% Marcação dos ângulos do triângulo retângulo
				\draw (D) ++(0.3,0) -- ++(0,0.3) -- ++(-0.3,0);
				\node[left] at (2.1,0.3) {\small $90^\circ$};
				
				\draw (A) ++(0.4,0) arc[start angle=0,end angle=60,radius=0.4];
				\node at (0.65,0.2) {\small $30^\circ$};
				
				\draw (C) ++(0,-0.75) arc[start angle=90,end angle=135,radius=-0.5];
				\node at (2.35,2.65) {\small $60^\circ$};
			\end{tikzpicture}
		\end{center}
		
		
		Para \(\theta = 30^\circ = \dfrac{\pi}{6}\):
		
		\begin{align*}
			\sin{\left(\dfrac{\pi}{6}\right)} &= \dfrac{\dfrac{l}{2}}{l} = \dfrac{1}{2} \\
			\cos{\left(\dfrac{\pi}{6}\right)} &= \dfrac{\dfrac{l\sqrt{3}}{2}}{l} = \dfrac{\sqrt{3}}{2} \\
			\tan{\left(\dfrac{\pi}{6}\right)} &= \dfrac{\dfrac{l}{2}}{\dfrac{l\sqrt{3}}{2}} = \dfrac{1}{\sqrt{3}} = \dfrac{\sqrt{3}}{3}
		\end{align*}
		
		
		Para \(\theta = 60^\circ = \dfrac{\pi}{3}\):

		\begin{align*}
			\sin{\left(\dfrac{\pi}{3}\right)} &= \dfrac{\dfrac{l\sqrt{3}}{2}}{l} = \dfrac{\sqrt{3}}{2} \\
			\cos{\left(\dfrac{\pi}{3}\right)} &= \dfrac{\dfrac{l}{2}}{l} = \dfrac{1}{2} \\
			\tan{\left(\dfrac{\pi}{3}\right)} &= \dfrac{\dfrac{l\sqrt{3}}{2}}{\dfrac{l}{2}} = \sqrt{3}
		\end{align*}
...
\begin{table}[h!]
	...
	\begin{tabular}{|c|c|c|c|}
		\hline
		\textbf{Função} & \(\mathbf{30^\circ = \dfrac{\pi}{6}}\) & \(\mathbf{45^\circ = \dfrac{\pi}{4}}\) & \(\mathbf{60^\circ = \dfrac{\pi}{3}}\) \\
		\hline
...
\renewcommand{\arraystretch}{1}
	...
	\begin{practice}
		...
		\begin{enumerate}
			\item[j)] $f(x) = \displaystyle \dfrac{1}{x}$ e $p$ qualquer
			...
			\begin{align*}
				...
				&= \dfrac{-x + p}{xp}\cdot \dfrac{1}{(x - p)} \quad \text{(Divisão de fração: multiplique pelo inverso)}\\
	...
	Intuitivamente, uma função contínua é aquela cujo gráfico pode ser desenhado sem tirar o lápis do papel. Observe a Fig.~\ref{fig:b} abaixo
		...
		\begin{subfigure}[t]{0.48\textwidth}
			...
			\begin{tikzpicture}
				\begin{axis}[
					...
					\draw[fill=green!50, fill opacity=0.5, draw=none] (axis cs:0,0.40) rectangle (axis cs:5,0.7340805);
					\draw[fill=green!50, fill opacity=0.5, draw=none] (axis cs:2.1,0) rectangle (axis cs:2.5,1.4);
					
					\addplot[smooth, thick, blue] {- ln(x)*cos(deg(x))};
					\node[blue] at (axis cs:4,0.91) [anchor=west] {$f$};
					\addplot+[only marks, mark=*, mark options={fill=blue}, blue, thick] coordinates {(2.2, 0.464008)};
					\addplot+[only marks, mark=*, mark options={fill=blue}, blue, thick] coordinates {(2.4, 0.6455651)};
					

				\end{axis}
			\end{tikzpicture}
			\caption{Função contínua}
			\label{fig:b}
		\end{subfigure}
		\hfill
		% Painel 2: função descontínua
		\begin{subfigure}[t]{0.48\textwidth}
			\centering
			\begin{tikzpicture}
				\begin{axis}[
					title={Função descontínua},
					axis lines=middle,
					xmin=1, xmax=4,
					ymin=0, ymax=1.3,
					xtick={2},
					...
					% Gráfico da função
					\addplot[domain=1:2, smooth, thick, blue] {-ln(x)*cos(deg(x))};
					
					\addplot[domain=2:4, smooth, thick, blue] {-ln(x)*cos(deg(x)) + 0.25};
					\node[blue] at (axis cs:4,0.91) [anchor=west] {$g$};
					
					% Adiciona os pontos de descontinuidade
					% Bolinha fechada em (2, f(2))
					\addplot+[only marks, mark=*, mark options={fill=blue}, blue] coordinates {(2, 0.287)};
					
					%% Cuidado: revisar a função, pois essa aqui é contínua. A bolinha deve estar acima do ponto p
					% Bolinha aberta em (2.5, f(2.5))
					\addplot+[only marks, mark=o, mark options={draw=blue}, blue, thick] coordinates {(2, 0.538451)};

				\end{axis}
			\end{tikzpicture}
			\caption{Gráfico da função descontínua}
			\label{fig:a}
		\end{subfigure}
		
		\caption{Comparação entre as funções: em (a), uma função contínua em $p$. Em (b), uma função descontínua em $p$.}
		\label{fig:comparacao-funcao-continua}
	\end{figure}
	
	Considere \textit{vizinhança }o intervalo ao redor do ponto $f(p)$. 
	
	Analisando a): no ponto $x$, veja que sua imagem ($f(x)$) está dentro da vizinhança do ponto $f(p)$. Isso vale para qualquer $x$ próximo a $p$. Ou seja, dentro da vizinhança de $f(p)$ existe uma vizinhança em $p$, para a qual $x$ dentro desse intervalo possui uma imagem $f(x)$ próxima a $f(p)$. Dizemos, então, que essa função é contínua.
	
	Analisando b): o mesmo não ocorre no gráfico da função $g$. Dizemos que essa função não é contínua.
	
	Podemos denotar este intervalo destacado em verde na Fig.~\ref{fig:comparacao-funcao-continua} como um intervalo. Veja a Fig.~\ref{fig:epsilon_delta}: 
	
		\begin{figure}[h]
		\centering
		\begin{tikzpicture}
			\begin{axis}[
				title={Aplicando um zoom na nossa função contínua},
				axis lines=middle,
				xmin=2, xmax=2.6,
				ymin=0.3, ymax=0.8,
				xtick={2.1, 2.2, 2.4, 2.5},
				xticklabels={\small $\delta - p$, $p$,$x$, \small $p + \delta$},
				ytick={0.40, 0.464008, 0.6455651, 0.7340805},
				yticklabels={\small $\epsilon - f(p)$, $f(p)$, $f(x)$, \small $\epsilon + f(p)$},
				samples=200,
				grid=both,
				enlargelimits=true,
				]
			...
			\end{axis}
		\end{tikzpicture}
		\caption{Função contínua. As vizinhanças agora podem ser denotadas por $\left[ \epsilon - f(p), f(p) + \epsilon \right]$ e $\left[ \delta - p, p + \delta \right]$, onde $\epsilon$ e $\delta$ são números reais.}
	...
	\begin{example}A partir de $| x - p |  < \delta \implies |f(x) - f(p) | < \epsilon$, mostre que $f(x) = 2x+1$ é contínua para qualquer $p$.
		...
		\begin{align*}
			...
			&\implies |x - p| < \dfrac{\epsilon}{2} \quad \text{(Sabemos que } |2| = 2 \text{)}
		\end{align*}
		
		Portanto, se tomarmos $\delta = \dfrac{\epsilon}{2}$
		
		
		\begin{align*}
			|x-p|\leq \delta \leq \dfrac{\epsilon}{2}| \implies f(x) - f(p)| \leq \dfrac{\epsilon}{2} \implies |x-p| \leq \dfrac{\epsilon}{2}
		\end{align*}
		...
		Mostramos que existe um $\delta = \dfrac{\epsilon}{2}$ que satisfaz a condição da definição de continuidade. 
		
		Logo, a função $f(x) = 2x + 1$ é contínua em todo ponto $p \in \mathbb{R}$.
		
		Experimente substituir valores quaisquer para $\delta$ e $\epsilon$.
		
				\begin{figure}[h]
			\centering
			\begin{tikzpicture}
				\begin{axis}[
					title={A função $f(x) = 2x + 1$ é contínua para todo $p \in \mathbb{R}$},
					axis lines=middle,
					xmin=0, xmax=2,
					ymin=1, ymax=4,
					xtick={0.5, 1, 1.2, 1.5}, %\delta = 1/2 \implies \epsilon = 1
					xticklabels={\scriptsize $\delta - p$, \scriptsize $p$, \scriptsize $x$, \scriptsize $p + \delta$},
					ytick={2,3,3.4,4},
					yticklabels={\scriptsize $\epsilon - f(p)$, \scriptsize $f(p)$, \scriptsize $f(x)$, \scriptsize $\epsilon + f(p)$},
					samples=200,
					enlargelimits=true,
					]
					
					\draw[fill=green!50, fill opacity=0.5, draw=none] (axis cs:0.5,0) rectangle (axis cs:1.5,5); %vertical
					\draw[fill=green!50, fill opacity=0.5, draw=none] (axis cs:0,2) rectangle (axis cs:3,4); % horizontal
					
					\addplot[smooth, thick, blue] {2*x + 1};
					\node[blue] at (axis cs:1.6,4) [anchor=west] {$f$};
					\addplot+[only marks, mark=*, mark options={fill=blue}, blue, thick] coordinates {(1, 3)};
					\addplot+[only marks, mark=*, mark options={fill=blue}, blue, thick] coordinates {(1.2, 3.4)};
					
					
				\end{axis}
			\end{tikzpicture}
			\caption{Gráfico de $f(x) = 2x + 1$, com $\epsilon = 1, \, \delta = \dfrac{1}{2}$.}
			\label{fig:f_linear_epsilon_delta}
		\end{figure}
	\end{example}
	
	\subsubsection{Aproximações pela esquerda ou direita de um ponto $x$}
		...
		\begin{subfigure}[t]{0.48\textwidth}
			...
			\begin{tikzpicture}
				\begin{axis}[
					...
					% Bolinha fechada em (2, f(2))
					\addplot+[only marks, mark=*, mark options={fill=blue}, blue] coordinates {(2, 0.287)};
					%% Observe novamente que essa função é contínua. A bolinha deve estar acia do ponto p.
					% Bolinha aberta em (2, g(2))
					\addplot+[only marks, mark=o, mark options={draw=blue}, blue, thick] coordinates {(2, 0.538451)};
		...
		\begin{subfigure}[t]{0.48\textwidth}
			...
			\begin{tikzpicture}
				\begin{axis}[
					...
					% Gráfico da função
					\addplot[domain=0:5, smooth, thick, blue] {x+1};
					\node[blue] at (axis cs:3.1,4) [anchor=west] {$h$};
					

					\addplot+[only marks, mark=*, mark options={fill=blue}, blue] coordinates {(1, 3)};
					
					% Bolinha aberta em (2.5, f(2.5))
					\addplot+[only marks, mark=o, mark options={draw=blue}, blue, thick] coordinates {(1, 2)};
					
				\end{axis}
			\end{tikzpicture}
			\caption{Gráfico da função descontínua, com $p \in D_h$}
			\label{fig:b2}
		\end{subfigure}
		
		\caption{Comparação entre as funções: em (a), uma função descontínua em $p$. Em (b), uma função descontínua em $p$.}
		\label{fig:comparacao-funcao-continua2}
	\end{figure}
	...
	\begin{snugshade}
		...
		Na Fig.~\ref{fig:comparacao-funcao-continua2} (b), $p \in D_h$. Contudo, a função é descontínua em $p$. Em outras palavras, $h(p)$ \textbf{está} definida para $p$, ainda que o valor esperado não seja o mesmo.
	...
	\begin{center}
			\begin{tikzpicture}
				\begin{axis}[
					title={Função descontínua, com $p \notin D_h$},
					axis lines=middle,
					xmin=0, xmax=4,
					ymin=0, ymax=4,
					xtick={1},
	...
	Já no gráfico da Fig. b), vemos que quando $x \to p$, $f(x)$ tende a um valor $L \neq f(p)$ , e a imagem de $p$ é $f(p)$.
	...
	\begin{figure}[h]
		...
		\begin{subfigure}[t]{0.48\textwidth}
			...
			\begin{tikzpicture}
				\begin{axis}[
					...
					% Gráfico da função
					\addplot[domain=0:5, smooth, thick, blue] {x+1};
					\node[blue] at (axis cs:3.1,4) [anchor=west] {$f$};
					
					\addplot+[only marks, mark=*, mark options={fill=blue}, blue, thick] coordinates {(1, 2)};
					\draw[->, red] (axis cs:1.5, 0.2) -- (axis cs:1.1, 0.2);
					\draw[->, red] (axis cs:0.5, 0.2) -- (axis cs:0.9, 0.2);
					\draw[->, red] (axis cs:0.5, 1.7) -- (axis cs:1, 2.2);
					\draw[->, red] (axis cs:1.5, 2.7) -- (axis cs:1, 2.2);
				\end{axis}
			\end{tikzpicture}
			\caption{}
			\label{fig:limite_1}
		...
		\begin{subfigure}[t]{0.9\textwidth}
			\centering
			%% Revisar conceito de continuidade. p não está no domínio da função f
			\begin{tikzpicture}
				\begin{axis}[
					axis lines=middle,
					xmin=0, xmax=4,
			...
			\label{fig:limite_3}
		\end{subfigure}
		\caption{Comparação entre os limites ($L$) das funções: em (a), uma função contínua em $p$ e $L=f(p)$. Em (b), uma função descontínua em $p$, com $f(p) \neq L$. Em (c), a função $f$ tende a $L$, ainda que $p$ não esteja no domínio.}
		\label{fig:comparacao-funcao-limites}
	\end{figure}
	...
	\begin{definition}[Limite de uma função]
		Seja $f: \mathbb{R} \longrightarrow \mathbb{R}$. O limite de uma função em que $x \to p$ é $L$ se, e somente se, para todo $\epsilon > 0$ existir $\delta > 0$, tal que, para todo $x \in D_f$, $0 < | x - p |  < \delta \implies |f(x) - L | < \epsilon$.
	\end{definition}
	...
	\subsection{Resumo de funções contínuas}
	
	As seguintes funções  abaixo são contínuas.
	...
	\begin{table}[h]
		...
		\begin{tabular}[t]{|c|c|}
			...
			\hline
			Polinomiais & $f(x) = mx + b$, $f(x) = ax^2 + bx + c$, etc...\\
			\hline
			Racionais & $f(x) = \dfrac{p(x)}{s(x)}, s(x) \neq 0$\\
			\hline
			Raízes & $f(x) = \sqrt[3]{x}$, $f(x) = \sqrt{x+1}$\\
			\hline
			Exponenciais & $f(x) = b^x, b >0, b\neq 1$, $f(x) = e^x$\\
			\hline
			Logarítmicas & $f(x) = \log x$, $f(x) = \ln{x}$\\
			\hline
			Trigonométricas & $f(x) = \sin x $, $f(x) = \cos x$, etc...\\
			\hline
		\end{tabular}
	\end{table}
	\renewcommand{\arraystretch}{1}
	...
	\subsection{Resolvendo exercícios}
	\textbf{Calcule e justifique.}

		
		$(a)$ Como a função $f(x) = x^{2}$ é contínua, temos que \begin{center}$\displaystyle{\lim_{x \to 2} x^{2}} = f(2) = 2^{2} = 4 \Longrightarrow \displaystyle{\lim_{x \to 2} x^{2}} = 4$\end{center}
		
		$(b)$ Como a função $f(x) = 3x + 1$ é contínua, temos que \begin{center}$\displaystyle{\lim_{x \to 1} (3x + 1)} = f(1) = 3.1 + 1 = 4 \Longrightarrow \displaystyle{\lim_{x \to 1} (3x + 1)} = 4$\end{center}
		
		$(c)$ Como a função $f(x) =  4x + 1$ é contínua, temos que \begin{center}$\displaystyle{\lim_{x \to -2} (4x + 1)} = f(-2) = 4.(-2) + 1 = -8 + 1 = -7 \Longrightarrow \displaystyle{\lim_{x \to -2} (4x + 1)} = -7$\end{center}
		
		$(d)$ Como a função $f(x) = 5$ é contínua, temos que \begin{center}$\displaystyle{\lim_{x \to 10} 5} = f(10) = 5 \Longrightarrow \displaystyle{\lim_{x \to 10} 5} = 5$\end{center}
		
		$(e)$ Como a função $f(x) = 50$ é contínua, temos que \begin{center}$\displaystyle{\lim_{x \to -9} 50} = f(-9) = 50 \Longrightarrow \displaystyle{\lim_{x \to -9} 50} = 50$\end{center}
		
		$(f)$ Como a função $f(x) = -x^{2} -2x + 3$ é contínua, temos que \\ \\ $\displaystyle{\lim_{x \to -1} (-x^{2} -2x + 3)} = f(-1) = -(-1)^{2} -2.(-1) + 3 = -1 + 2 + 3 = 4 \Rightarrow \displaystyle{\lim_{x \to -1} (-x^{2} -2x + 3)} = 4$ \\
		
		$(g)$ Como a função $f(x) = \sqrt{x}$ é contínua, temos que \begin{center}$\displaystyle{\lim_{x \to 4}\sqrt{x}} = f(4) = \sqrt{4} = 2 \Longrightarrow \displaystyle{\lim_{x \to 4} \sqrt{x}} = 2$\end{center}
		
		$(h)$ Como a função $f(x) = \sqrt[3]{x}$ é contínua, temos que \begin{center}$\displaystyle{\lim_{x \to -3}\sqrt[3]{x}} = f(-3) = \sqrt[3]{-3} \Longrightarrow \displaystyle{\lim_{x \to -3} \sqrt[3]{x}} = \sqrt[3]{-3}$\end{center}
		
		$(i)$ Como a função $f(x) = \sqrt{5}$ é contínua, temos que \begin{center}$\displaystyle{\lim_{x \to -8}\sqrt{5}} = f(-8) = \sqrt{5} \Longrightarrow \displaystyle{\lim_{x \to -8} \sqrt{5}}=\sqrt{5}$\end{center}
		
		$(j)$ A função $f(x) = \dfrac{x^{2} - 9}{x - 3}$ não está definida para $x = 3$, entretanto, podemos manipular a expressão e obter uma equivalente $g(x)$, isto é, $f(x) = g(x)$ para $x \neq 3$, com $g(x)$ contínua em $x = 3$ e concluir que $\displaystyle{\lim_{x \to 3} f(x)}=\displaystyle{\lim_{x \to 3} g(x)}=g(3)$. Deste modo, temos que \begin{center}$f(x) = \dfrac{x^{2} - 9}{x - 3} = \dfrac{(x - 3).(x + 3)}{x - 3} = x + 3$, para $x \neq 3$\end{center} Tomando $g(x) = x + 3$, note que $f(x)=g(x)$ para $x \neq 3$ e que $g(x)$ é contínua em $x = 3$, logo \begin{center}$\displaystyle{\lim_{x \to 3} f(x)} = \displaystyle{\lim_{x \to 3} \dfrac{x^{2} - 9}{x - 3}}=\displaystyle{\lim_{x \to 3} (x + 3)} = g(3) = 3 + 3 = 6 \Longrightarrow \displaystyle{\lim_{x \to 3} \dfrac{x^{2} - 9}{x - 3}} = 6$\end{center}
		
		Os próximos exercícios em que forem feitas manipulações algébricas como no item anterior possuem mesma justificativa e iremos omiti-la para facilitar a leitura da resolução.
		
		$(l)$ Como a função $f(x) = \dfrac{x^{2} - 9}{x + 3}$ é contínua, temos que \begin{center}$\displaystyle{\lim_{x \to 3} \dfrac{x^{2} - 9}{x + 3}} = f(3) = \dfrac{3^{2} - 9}{3 + 3} = \dfrac{9 - 9}{6} = \dfrac{0}{6} = 0 \Longrightarrow \displaystyle{\lim_{x \to 3} \dfrac{x^{2} - 9}{x + 3}} = 0$\end{center}
		
		$(m)$ Como a função $f(x) = \dfrac{x^{2}-9}{x - 3}$ é contínua, temos que \begin{center}$\displaystyle{\lim_{x \to -1} \dfrac{x^{2} - 9}{x - 3}} = f(-1) = \dfrac{(-1)^{2} - 9}{-1 - 3} = \dfrac{1-9}{-4} = \dfrac{8}{4} = 2 \Longrightarrow \displaystyle{\lim_{x \to -1} \dfrac{x^{2} - 9}{x + 3}} = -4$\end{center}
		
		$(n)$ É preciso manipular esta função, portanto \begin{center}$\displaystyle{\lim_{x \to \frac{1}{2}} \dfrac{4x^{2} - 1}{2x - 1}} = \displaystyle{\lim_{x \to \frac{1}{2}} \dfrac{(2x - 1)(2x + 1)}{2x - 1}} = \displaystyle{\lim_{x \to \frac{1}{2}} (2x + 1)} = 2.\dfrac{1}{2} + 1 = 1 + 1 = 2 \Longrightarrow \displaystyle{\lim_{x \to \frac{1}{2}} \dfrac{4x^{2} - 1}{2x - 1}} = 2$\end{center}
		
		$(o)$ É preciso manipular esta função, portanto \begin{center}$\displaystyle{\lim_{x \to 1} \dfrac{\sqrt{x} - 1}{x - 1}} = \displaystyle{\lim_{x \to 1} \dfrac{\sqrt{x} - \sqrt{1}}{(\sqrt{x} - \sqrt{1})(\sqrt{x} + \sqrt{1})}} = \displaystyle{\lim_{x \to 1} \dfrac{1}{\sqrt{x} + 1}} = \dfrac{1}{\sqrt{1} + 1} = \dfrac{1}{2} \Rightarrow \displaystyle{\lim_{x \to 1} \dfrac{\sqrt{x} - 1}{x - 1}} = \dfrac{1}{2}$\end{center}
		
		$(p)$ É preciso manipular esta função, portanto \\ \\ $\displaystyle{\lim_{x \to -\frac{1}{3}} \dfrac{9x^{2} - 1}{3x + 1}} = \displaystyle{\lim_{x \to -\frac{1}{3}} \dfrac{(3x - 1)(3x + 1)}{3x + 1}} = \displaystyle{\lim_{x \to -\frac{1}{3}} (3x - 1)} = 3.\biggl(-\dfrac{1}{3}\biggl) - 1 = -1 - 1 = -2 \Longrightarrow \\ \\ \\ \Longrightarrow \displaystyle{\lim_{x \to -\frac{1}{3}} \dfrac{9x^{2} - 1}{3x + 1}} = -2$ \\
		
		$(q)$ É preciso manipular esta função, portanto \\ \\ $\displaystyle{\lim_{x \to 3} \dfrac{\sqrt{x} - \sqrt{3}}{x - \sqrt{3}}} = \displaystyle{\lim_{x \to 3} \dfrac{\sqrt{x} - 3}{(\sqrt{x} - \sqrt{3})(\sqrt{x} + \sqrt{3})}} = \displaystyle{\lim_{x \to 3} \dfrac{1}{\sqrt{x} + \sqrt{3}}} = \dfrac{1}{\sqrt{3} + \sqrt{3}} = \dfrac{1}{2\sqrt{3}} \Longrightarrow \\ \\ \\ \Longrightarrow \displaystyle{\lim_{x \to 3} \dfrac{\sqrt{x} - \sqrt{3}}{x - 3}} = \dfrac{1}{2\sqrt{3}}$ \\
		
		$(r)$ É preciso manipular esta função, portanto \\ \\ $\displaystyle{\lim_{x \to 3} \dfrac{\sqrt[3]{x} - \sqrt[3]{3}}{x - 3}} = \displaystyle{\lim_{x \to 3} \dfrac{\sqrt[3]{x} - \sqrt[3]{3}}{(\sqrt[3]{x} - \sqrt[3]{3})(\sqrt[3]{x^2} + \sqrt[3]{x3} + \sqrt[3]{3^{2}})}} = \displaystyle{\lim_{x \to 3} \dfrac{1}{\sqrt[3]{x^2} + \sqrt[3]{3x} + \sqrt[3]{9}}} = \dfrac{1}{\sqrt[3]{3^2} + \sqrt[3]{3.3} + \sqrt[3]{9}} = \\ \\ \\ = \dfrac{1}{3\sqrt[3]{9}} \Longrightarrow \displaystyle{\lim_{x \to 3} \dfrac{\sqrt[3]{x} - \sqrt[3]{3}}{x - 3}} = \dfrac{1}{3\sqrt[3]{9}}$ \\
		
		$(s)$ É preciso manipular esta função, portanto \\ \\ $\displaystyle{\lim_{x \to 2} \dfrac{\sqrt[4]{x} - \sqrt[4]{2}}{x - 2}} = \displaystyle{\lim_{x \to 2} \dfrac{\sqrt[4]{x} - \sqrt[4]{2}}{(\sqrt[4]{x} - \sqrt[4]{2})(\sqrt[4]{x^3} + \sqrt[4]{x^{2}2} + \sqrt[4]{x2^{2}} + \sqrt[4]{2^{3}})}} = \displaystyle{\lim_{x \to 2} \dfrac{1}{\sqrt[4]{x^3} + \sqrt[4]{2x^{2}} + \sqrt[4]{4x} + \sqrt[4]{8}}} = \\ \\ \\ \dfrac{1}{\sqrt[4]{2^3} + \sqrt[4]{2.2^{2}} + \sqrt[4]{4.2} + \sqrt[4]{8}} = \dfrac{1}{4\sqrt[4]{8}} \Longrightarrow \displaystyle{\lim_{x \to 2} \dfrac{\sqrt[4]{x} - \sqrt[4]{2}}{x - 2}} = \dfrac{1}{4\sqrt[4]{8}}$ \\
		
		$(t)$ Como a função $f(x) = \dfrac{x^{2} +3x - 1}{x^{2} + 2}$ é contínua, temos que \begin{center}$\displaystyle{\lim_{x \to 0} \dfrac{x^{2} +3x - 1}{x^{2} + 2}} = f(0) = \dfrac{0^{2} +3.0 - 1}{0^{2} + 2} = -\dfrac{1}{2} \Longrightarrow \displaystyle{\lim_{x \to 0} \dfrac{x^{2} +3x - 1}{x^{2} + 2}} = -\dfrac{1}{2}$\end{center}
		
		$(u)$ É preciso manipular esta função, portanto \\ \\ $\displaystyle{\lim_{x \to 1} \dfrac{\sqrt{x} - 1}{\sqrt{2x + 3}-\sqrt{5}}} = \displaystyle{\lim_{x \to 1} \dfrac{\sqrt{x} - 1}{\sqrt{2x + 3}-\sqrt{5}}.\dfrac{\sqrt{2x + 3}+\sqrt{5}}{\sqrt{2x + 3}+\sqrt{5}}} = \displaystyle{\lim_{x \to 1} \dfrac{(\sqrt{x} - 1).(\sqrt{2x + 3}+\sqrt{5})}{2x + 3 - 5}} = \\ \\ \\ = \displaystyle{\lim_{x \to 1} \dfrac{(\sqrt{x} - 1).(\sqrt{2x + 3}+\sqrt{5})}{2x - 2}} = \displaystyle{\lim_{x \to 1} \dfrac{(\sqrt{x} - 1).(\sqrt{2x + 3}+\sqrt{5})}{2(x - 1)}} = \displaystyle{\lim_{x \to 1} \dfrac{(\sqrt{x} - \sqrt{1}).(\sqrt{2x + 3}+\sqrt{5})}{2(\sqrt{x} - \sqrt{1})(\sqrt{x}+\sqrt{1})}} = \\ \\ = \displaystyle{\lim_{x \to 1} \dfrac{\sqrt{2x + 3}+\sqrt{5}}{2(\sqrt{x}+\sqrt{1})}} = \dfrac{\sqrt{2.1 + 3}+\sqrt{5}}{2(\sqrt{1}+1)} = \dfrac{\sqrt{5}+\sqrt{5}}{4} = \dfrac{2\sqrt{5}}{4} = \dfrac{\sqrt{5}}{2}$
		
	
	\textbf{Exercício 2 (Exercício 4, a) do livro). Calcule $\displaystyle \lim_{h \to 0} \dfrac{f(x+h) - f(x)}{h}$, sendo $f$ dada por $f(x) = x^2$.}
	
	\begin{align*}
		\lim_{h \to 0} \dfrac{f(x+h) - f(x)}{h} &= \lim_{h \to 0} \dfrac{(x+h)^2 - x^2}{h} \\
		&= \lim_{h \to 0} \dfrac{x^2 + 2xh + h^2 - x^2}{h} \\
		&= \lim_{h \to 0} \dfrac{(\cancel{h})(2x + h)}{\cancel{h}} \quad \text{($h$ em evidência. Simplificamos)}\\
	...
	\begin{definition}[Derivada de uma função]\label{def:derivada}
		...
		\begin{equation}
			\boxed{f'(p) = \lim_{x \to p} \dfrac{f(x) - f(p)}{x-p}}
		\end{equation}
	...
	\subsubsection{A derivada como um limite}
	...
	\begin{example}Seja a função $f: \mathbb{R} \longrightarrow \mathbb{R}$, tal que $f(x) = 2$. Calcule a derivada $f'(x)$.
		...
		\begin{align*}
			f’(p) &= \lim_{x \to p} \dfrac{f(x) - f(p)}{x - p} \\
			&= \lim_{x \to p} \dfrac{2 - 2}{x - p} \\
			&= \lim_{x \to p} \dfrac{0}{x - p} \\
			&= \lim_{x \to p} 0 \\
			&= 0
		\end{align*}  
	...
	\begin{example}
		...
		\begin{align*}
			f'(p) &= \lim_{x \to p} \dfrac{f(x) - f(p)}{x-p} \\
			&= \lim_{x \to p} \dfrac{(2x + 1) - (2p + 1)}{x-p} \\
			&= \lim_{x \to p} \dfrac{2x + 1 - 2p - 1}{x-p} \\
			&= \lim_{x \to p} \dfrac{2(x - p)}{x-p} \\
			&= \lim_{x \to p} 2 \\
			&= 2
		\end{align*}
	...
	\begin{example}
		...
		\begin{align*}
			f'(p) &= \lim_{x \to p} \dfrac{f(x) - f(p)}{x-p} \\
			&= \lim_{x \to p} \dfrac{x^2 - p^2}{x-p} \\
			&= \lim_{x \to p} \dfrac{(x + p)(x - p)}{x-p} \\
			&= \lim_{x \to p} (x + p) \\
			&= p + p \\
			&= 2p
		\end{align*}
	...
	\begin{example}
		...
		\begin{align*}
			f'(p) &= \lim_{x \to p} \dfrac{f(x) - f(p)}{x-p} \\
			&= \lim_{x \to p} \dfrac{x^3 - p^3}{x-p} \\
			&= \lim_{x \to p} \dfrac{(x - p)(x^2 + xp + p^2)}{x-p} \\
	...
	\subsubsection{A equação da reta tangente a um gráfico}
	\begin{snugshade}
		\textbf{\textcolor{blue}{Relembre!}}
		A equação fundamental de uma reta e dada pela expressão
		\begin{equation*}
			y - y_0 = m(x - x_0)
		\end{equation*}
		Onde $(x_0, y_0)$ são as coordenadas de um ponto conhecido, e $m$ é o coeficiente angular desta reta.
	\end{snugshade}
	
	A reta tangente no ponto $(p, f(p))$ será dada por
	
	\begin{align*}
		\Aboxed{y - f(p) &= f'(p) \cdot (x - p)} \quad \text{ou} \quad \Aboxed{y - f(x_0) &= f'(x_0) \cdot (x - p)}
	\end{align*}
	
	Onde $f(p) = y_0$, $p=x_0$ e $f'(p) = m$.
	
	\begin{example}
		Determine a equação da reta tangente a $f(x) = x^2$ em $p=3$.
		...
		Uma equação dada no formato $y=mx + n$, onde $m$ e $n$ são números reais, é chamada de \textit{equação reduzida da reta}.
	\end{example} 
	
	\begin{example}
		Seja $f(x) = \sqrt{x}$. Calcule a derivada.
	
		
		Usamos a versão alternativa da definição de derivada.
		
		$$
		f'(x) = \lim_{h \to 0} \dfrac{f(x+h) - f(x)}{h} = \lim_{h \to 0} \dfrac{\sqrt{x+h} - \sqrt{x}}{h}
		$$
		
		Multiplicamos pelo conjugado:
		
		\begin{align*}
			f'(x) &= \lim_{h \to 0} \dfrac{\sqrt{x+h} - \sqrt{x}}{h} \cdot \dfrac{\sqrt{x+h} + \sqrt{x}}{\sqrt{x+h} + \sqrt{x}} \\
			&= \lim_{h \to 0} \dfrac{(x+h) - x}{h(\sqrt{x+h} + \sqrt{x})} \\
			&= \lim_{h \to 0} \dfrac{\cancel{h}}{\cancel{h}(\sqrt{x+h} + \sqrt{x})} \\
			&= \lim_{h \to 0} \dfrac{1}{\sqrt{x+h} + \sqrt{x}} \\
	...
	\subsubsection{Regras de derivação}
	A partir desta seção, demonstra-se algumas das regras de derivação.
	
	Cada uma das regras de derivação pode ser demonstrada. Como não é o intuito do curso, iremos demonstrar apenas a regra da soma de duas funções.
	
	\begin{proof}[A derivada da soma de duas funções]
		
		Sejam \( f \) e \( g \) funções deriváveis em \( p \in D_f\, \text{e}\, D_g \), e definimos \( h(x) = f(x) + g(x) \). Vamos provar que:
		...
		\begin{align*}
			h'(p) &= \lim_{x \to p} \dfrac{h(x) - h(p)}{x - p} \quad \text{(Substituindo $h(x) = f(x) + g(x)$)}\\
			&= \lim_{x \to p} \dfrac{[f(x) + g(x)] - [f(p) + g(p)]}{x - p} \\
			&= \lim_{x \to p} \dfrac{[f(x) - f(p)] + [g(x) - g(p)]}{x - p} \\
			&= \lim_{x \to p} \left( \dfrac{f(x) - f(p)}{x - p} + \dfrac{g(x) - g(p)}{x - p} \right) \quad \text{(Propriedade dos limites)}\\
			&= \lim_{x \to p} \dfrac{f(x) - f(p)}{x - p} + \lim_{x \to p} \dfrac{g(x) - g(p)}{x - p} \\
			&= f'(p) + g'(p) \qedhere
		\end{align*}
	...
	\subsubsection{Tabela de derivação}
	
	\renewcommand{\arraystretch}{2.5}
	\begin{table}[h]
		\centering
		\caption{Caixa de ferramentas: regras comuns de derivação}
		\begin{tabular}{|>{\centering\arraybackslash}m{3cm}|>{\centering\arraybackslash}m{10cm}|}
			...
			\hline
			Produto & $\displaystyle [p(x) \cdot s(x)]' = p'(x)s(x) + p(x)s'(x)$ \\
			\hline
			Quociente & $\displaystyle \left[\dfrac{p(x)}{s(x)}\right]' = \dfrac{p'(x)s(x) - p(x)s'(x)}{[s(x)]^2}, s(x) \neq 0$ \\
			\hline
		\end{tabular}
	\end{table}
	\renewcommand{\arraystretch}{1.0}
	
	\subsubsection{Lista de derivação}
	
	Incluímos outros recursos na nossa caixa de ferramentas matemáticas para derivação:
	%%Inserir quadro de derivação do sen cos
		\renewcommand{\arraystretch}{1.5}
		
		\begin{table}[h!]
			\centering
			\caption{Caixa de ferramentas: principais regras de derivação}
			\begin{tabular}{|>{\centering\arraybackslash}m{5cm}|>{\centering\arraybackslash}m{8cm}|}
				...
				\hline
				$\displaystyle f(x) = \ln x$ & $\displaystyle f'(x) = \dfrac{1}{x}$ \\
				\hline
			\end{tabular}
	...
	\textbf{Exercício 1. Calcule \(f'(x)\).}
	
	\begin{itemize}
		\item[a)] \(f(x) = 3x^2 + 5\)
		\item[b)] \(f(x) = x^3 + x^2 + 1\)
		\item[c)] \(f(x) = 3x^3 - 2x^4\)
		\item[d)] \(f(x) = 3x + \sqrt{x}\)
		\item[e)] \(f(x) = 5 + 3x^{-2}\)
		\item[f)] \(f(x) = 2\sqrt{x}\)
		\item[g)] \(f(x) = 3x + \tfrac{1}{x}\)
		\item[h)] \(f(x) = \tfrac{4}{x} + \tfrac{5}{x^2}\)
		\item[i)] \(f(x) = \tfrac{2}{3}x^3 + \tfrac{1}{4}x^2\)
		\item[j)] \(f(x) = \sqrt[3]{x} + \sqrt{x}\)
		\item[k)] \(f(x) = \tfrac{4}{x} + \tfrac{\sqrt{x}}{x^2}\)
		\item[l)] \(f(x) = 2x + \tfrac{1}{x^3} + \tfrac{1}{x^2}\)
		\item[m)] \(f(x) = 6x^3 + \sqrt[3]{x}\)
		\item[n)] \(f(x) = 5x^4 + bx^3 + cx^2 + k\), onde \(b, c\) e \(k\) são constantes.
	\end{itemize}
	
	\textbf{Soluções:}
	%% Tirar pontos finais, colocar soluções em raízes
	\begin{itemize}
		\item[a)] \(f(x) = 3x^2 + 5\)  
		...
		\item[h)] \(f(x) = \dfrac{4}{x} + \tfrac{5}{x^2}\)  
		
		
		\[
		f'(x)
		= 4\cdot(-1)x^{-2} + 5\cdot(-2)x^{-3}
		= -4x^{-2} - 10x^{-3}
		= -\dfrac{4}{x^2} - \dfrac{10}{x^3}
		\]
		
		
		
		\item[i)] \(f(x) = \dfrac{2}{3}x^3 + \dfrac{1}{4}x^2\)  
		
		
		\[
		f'(x)
		= \dfrac{2}{3}\cdot3x^2 + \dfrac{1}{4}\cdot2x
		= 2x^2 + \tfrac12 x
		\]
		...
		\[
		f'(x)
		= \dfrac{1}{3}x^{-2/3} + \dfrac12 x^{-1/2}
		\]
		
		
		
		\item[k)] \(f(x) = \dfrac{4}{x} + \dfrac{\sqrt{x}}{x^2}\)  
		
		
		\[
		f'(x)
		= 4\cdot(-1)x^{-2} + \dfrac{d}{dx}\bigl(x^{1/2}\,x^{-2}\bigr)
		= -4x^{-2} + \dfrac{d}{dx}(x^{-3/2})
		= -4x^{-2} - \dfrac32 x^{-5/2}
		\]
		
		
		
		\item[l)] \(f(x) = 2x + \dfrac{1}{x^3} + \dfrac{1}{x^2}\)  
		...
		\[
		f'(x)
		= 6\cdot3x^2 + \dfrac{1}{3}x^{-2/3}
		= 18x^2 + \dfrac{1}{3}x^{-2/3}
		\]
		
		
		
		\item[n)] \(f(x) = 5x^4 + bx^3 + cx^2 + k\)  
		
		
		\[
		f'(x)
		= 5\cdot4x^3 + b\cdot3x^2 + c\cdot2x + 0
		= 20x^3 + 3b\,x^2 + 2c\,x
		\]
	...
	\textbf{Exercício 2}. Seja $g(x) = x^3 + \dfrac{1}{x}$. Determine a equação da reta tangente ao gráfico de \(g\) no ponto \(\bigl(1,\,g(1)\bigr)\).
	...
	\begin{align*}
		g'(x) &= 3x^2 - x^{-2}, \\
		g'(1) &= 3\cdot1^2 - 1^{-2} = 3 - 1 = 2, \\
		g(1) &= 1^3 + \dfrac{1}{1} = 1 + 1 = 2.
	\end{align*}
	
	O ponto de tangência é \((x_0,y_0) = (1,2)\).  
	A inclinação da reta tangente é \(m = g'(1) = 2\).  
	
	A equação da reta tangente, usando a forma fundamental, é
	...
	\begin{itemize}
		\item[a)] \( 3x^2 + 5\cos x \)
		\item[b)] \( \dfrac{\cos x}{x^2 + 1} \)
		\item[c)] \( x \sin x \)
		\item[d)] \( x^2 \tan x \)
		\item[e)] \( \dfrac{x + 1}{\tan x} \)
		\item[f)] \( \dfrac{3}{\sin x + \cos x} \)
	\end{itemize}
	...
	\begin{itemize}
		...
		\item[b)] \( f(x) = \dfrac{\cos x}{x^2 + 1} \)
		
		
		\[
		f'(x)
		= \dfrac{(x^2 + 1)(-\sin x) - \cos x \cdot 2x}{(x^2 + 1)^2}
		= \dfrac{-(x^2 + 1)\sin x - 2x\cos x}{(x^2 + 1)^2}.
		\]
		...
		\[
		f'(x)
		= 2x\tan x + x^2\cdot\sec^2 x.
		\]
		
		
		
		\item[e)] \( f(x) = \dfrac{x + 1}{\tan x} \)
		
		
		\[
		f'(x)
		= \dfrac{\tan x \cdot (x + 1)' - (x + 1)\cdot(\tan x)'}{(\tan x)^2}
		= \dfrac{\tan x - (x + 1)\sec^2 x}{\tan^2 x}.
		\]
		
		
		
		\item[f)] \( f(x) = \dfrac{3}{\sin x + \cos x} \)
		
		
		\[
		f'(x)
		= -3\cdot\dfrac{\cos x - \sin x}{(\sin x + \cos x)^2}.
		\]
	...
	\subsection{Regra da cadeia para derivação de funções compostas}
	
	\begin{snugshade}
		\textbf{\textcolor{blue}{Relembre!}} Funções compostas: p.~\pageref{subsec:funcoes_compostas}.
	\end{snugshade}
	
	
		Sejam $f$ e $g$ duas funções deriváveis, tal que $Im_g \subseteq D_f$. Podemos montar uma função composta $h(x) = f(g(x))$.
	...
	\begin{table}[h]
		...
		\begin{tabular}{|>{\centering\arraybackslash}m{3cm}|>{\centering\arraybackslash}m{10cm}|}
			...
			Produto & $\displaystyle [p(x) \cdot s(x)]' = p'(x)s(x) + p(x)s'(x)$ \\
			\hline
			Quociente & $\displaystyle \left[\dfrac{p(x)}{s(x)}\right]' = \dfrac{p'(x)s(x) - p(x)s'(x)}{[s(x)]^2}$ \\
			\hline
			Cadeia & $\displaystyle [p(s(x))]' = p'(s(x)) \cdot s'(x)$\\
			\hline
		\end{tabular}
	\end{table}
	\renewcommand{\arraystretch}{1.0}
	...
	Derivadas são ferramentas úteis para analisar o comportamento de funções ou esboçar seu gráfico. Vejamos o exemplo a seguir:
		...
		\begin{example}
			Seja $f(x) = x^2 + 6x + 9$. Queremos descobrir quando ela cresce, decresce, atinge um ponto máximo ou um ponto mínimo.
			
			\textit{Passo 1:} Calculando a derivada de $f(x)$:
			...
			\textit{Passo 2:} Encontrando os pontos críticos onde $f'(x) = 0$:
			...
			\textit{Passo 3:} Analisando o sinal de $f'(x)$:
			
			Para $x < -3$, resolvendo em $x = -4$: $f'(-4) = -2$, então $f$ é decrescente.
			
			Para $x > -3$, resolvendo em $x = 0$: $f'(0) = 6$, então $f$ é crescente.
			
			\textit{Passo 4:} Analisando o comportamento no ponto crítico $x = -3$:
			
			Repare como, em $x = -3$, o sinal de $f'(x)$ muda de negativo para positivo.
			
			Podemos confirmar com o teste da segunda derivada da função
			
			\begin{table}[h]
				\centering
				\begin{tabular}[t]{|c|c|}
					\hline
					$f'(x) = 0$ e $f''(x) > 0$ & Ponto de mínimo local\\
					\hline
					$f'(x) = 0$ e $f''(x) < 0$ & Ponto de máximo local\\
					\hline
				\end{tabular}
			\end{table}
			...
			\begin{figure}[h]
				...
				\begin{tikzpicture}
					\begin{axis}[
						...
						% Função f(x)
						\addplot [domain=-6:2, thick, blue] {x^2 + 6*x + 9};
						\node[blue] at (axis cs:0.5, 10) {$f$};
						
						% Derivada f'(x)
						\addplot [thick, dashed, red] {2*x + 6} node[above right] {$f'$};
						\node[red] at (axis cs:0.5, 6) {$f'$};
						
						% Ponto de mínimo
						\addplot[only marks,mark=*,mark size=3pt,color=black] coordinates {(-3,0)};
						
						% Indicação de crescimento e decrescimento
						\draw[->,red, shorten <=2pt, thick] (axis cs:-5.2,1.5) -- (axis cs:-4.2,0.1);
						\draw[->,red, shorten <=2pt, thick] (axis cs:-2,.1) -- (axis cs:-1,1.5);
						
					\end{axis}
				\end{tikzpicture}
				\caption{Analisando a função $f(x) = x^2 + 6x + 9$}
			\end{figure}
		\end{example}
		...
		\begin{table}[h]
			\centering
			\caption{Estudando o comportamento da função.}
			\begin{tabular}{|>{\centering\arraybackslash}m{6cm}|>{\centering\arraybackslash}m{8cm}|}
				\hline
				$\displaystyle f'(x) = 0$ & Candidatos a pontos de máximo e mínimo \\
				\hline
				$\displaystyle f'(x) > 0$ & A função $f(x)$ é crescente \\
				\hline
				$\displaystyle f'(x) < 0$ & A função $f(x)$ é decrescente \\
				\hline
				$\displaystyle f''(x) > 0$ & Concavidade para cima \\
				\hline
				$\displaystyle f''(x) < 0$ & Concavidade para baixo\\
				\hline
				$\displaystyle f''(x) = 0$ & Candidato a ponto de inflexão\\
				\hline
			\end{tabular}
		\end{table}
	...
	\textbf{Exercício 1.} Determine a derivada.
	
	$(a)$ $y' = (cos \ 4x).4 = 4cos \ 4x \Longrightarrow y' = 4cos \ 4x$ \\
		
		$(b)$ $y' = (-sen \ 5x).5 = -5sen \ 5x \Longrightarrow y' = -5sen \ 5x$ \\
		
		$(c)$ $f'(x) = (e^{3x}).3 = 3e^{3x} \Longrightarrow f'(x) = 3e^{3x}$ \\
		
		$(d)$ $f'(x) = (-sen \ 8x).8 = -8sen \ 8x \Longrightarrow f'(x) = -8sen \ 8x$ \\
		
		$(e)$ $y' = (cos \ t^{3}).3t^{2} = 3t^{2}cos \ t^{3} \Longrightarrow y' = 3t^{2}cos \ t^{3}$ \\
		
		$(f)$ $g'(t) = \dfrac{1}{2t + 1}.2 = \dfrac{2}{2t + 1} \Longrightarrow g'(t) = \dfrac{2}{2t + 1}$ \\
		
		$(g)$ $x' = e^{sen \ t}cos \ t$ \\
		
		$(h)$ $f'(x) = (-sen \ e^{x}).e^{x} = -e^{x}sen \ e^{x} \Longrightarrow f'(x) = -e^{x}sen \ e^{x}$ \\
		
		$(i)$ $y' = 3(sen \ x + cos \ x)^{2}.(cos \ x - sen \ x)$ \\
		
		$(j)$ $y' = \dfrac{1}{2\sqrt{3x + 1}}.3 = \dfrac{3}{2\sqrt{3x + 1}} \Longrightarrow y' = \dfrac{3}{2\sqrt{3x + 1}}$ \\
		
		$(l)$ $f'(x) = \dfrac{1}{3\sqrt[3]{\biggl( \dfrac{x - 1}{x + 1} \biggl)^{2}}}.\dfrac{1.(x + 1) - (x - 1).1}{(x + 1)^{2}} = \dfrac{2}{3(x + 1)^{2}\sqrt[3]{\biggl( \dfrac{x - 1}{x + 1} \biggl)^{2}}} = \dfrac{2}{3(x + 1)^{2}}\sqrt[3]{\biggl( \dfrac{x + 1}{x - 1} \biggl)^{2}} \Longrightarrow \\ \\ \Longrightarrow f'(x) = \dfrac{2}{3(x + 1)^{2}}\sqrt[3]{\biggl( \dfrac{x + 1}{x - 1} \biggl)^{2}}$ \\
		
		$(m)$ $y' = (e^{-5x}).(-5) = -5e^{-5x} \Longrightarrow y' = -5e^{-5x}$ \\
		
		$(n)$ $x' = \dfrac{1}{t^{2} + 3t + 9}.(2t + 3 + 0) = \dfrac{2t + 3}{t^{2} + 3t + 9} \Longrightarrow x' = \dfrac{2t + 3}{t^{2} + 3t + 9}$ \\
		
		$(o)$ $f'(x) = e^{tg \ x}sec^{2} \ x$ \\
		
		$(p)$ $y' = cos \ (cos \ x).(-sen \ x) = -sen \ xcos \ (cos \ x) \Longrightarrow y' = -sen \ xcos \ (cos \ x)$ \\
		
		$(q)$ $g'(t) = 4(t^{2} + 3)^{3}.2t = 8t(t^{2} + 3)^{2} \Longrightarrow g'(t) = 8t(t^{2} + 3)^{2}$ \\
		
		$(r)$ $f'(x) = -sen \ (x^{2} + 3).(2x) = -2xsen \ (x^{2} + 3)$ \\
		
		$(s)$ $y' = \dfrac{1}{2\sqrt{x + e^{x}}}.(1 + e^{x}) = \dfrac{1 + e^{x}}{2\sqrt{x + e^{x}}} \Longrightarrow y' = \dfrac{1 + e^{x}}{2\sqrt{x + e^{x}}}$ \\
		
		$(t)$ $y' = sec^{2} \ 3x.(3) = 3sec^{2} \ 3x \Longrightarrow y' = 3sec^{2} \ 3x$ \\
		
		$(u)$ $y' = sec \ 3x.tg \ 3x.(3) = 3sec \ 3x.tg \ 3x \Longrightarrow y' = 3sec \ 3x.tg \ 3x$ \\
		
	
	\begin{snugshade}
		\textbf{\textcolor{violet}{Saiba mais!}}
		
		\textbf{O Teorema do Valor Médio (TVM)} nos diz que, se uma função $f$ é contínua no intervalo $[a,b]$, e houver um ponto $c$ entre $a$ e $b$, então $c$ possui uma imagem $f(c)$ que está entre $f(a)$ e $f(b)$.
		...
		\begin{center}
			...
			\begin{axis}[
				...
				ytick={4, 9, 16},
				xticklabels = {$a$, $c$, $b$},
				yticklabels = {$f(a)$, $f(c)$, $f(b)$},
				domain=-10:10,
				grid=both,
				samples=200,
				]
				
				% Função f(x) genérica, vamos usar f(x) = x^2 como exemplo
				\addplot [thick, blue, name path=f] {x^2};
				
				% Marcando f(a) e f(b)
				\addplot[only marks,mark=*,mark size=3pt,color=black] coordinates {(2,4)};
				\addplot[only marks,mark=*,mark size=3pt,color=black] coordinates {(4,16)};
				
				
				% Linhas verticais para a e b
				\draw[dotted] (axis cs:2,0) -- (axis cs:2,4);
				\draw[dotted] (axis cs:4,0) -- (axis cs:4,16);
			\end{axis}
		\end{tikzpicture}
		\end{center}
		
		
		%%Copiar a definição do Guidorizzi
		\begin{theorem}[Teorema do Valor Médio]
			Se $f$ for contínua em um intervalo $[\,a,b\,]$, e $f$ for derivável em $]\,a,b\,[$, então existe $c \in ]\,a,b\,[$ tal que
			\begin{equation*}
	...
	\subsection{Integrais}\label{subsec:integrais}
	...
	\begin{center}
		\begin{tikzpicture}
			\begin{axis}[
				...
				% Função f(x)
				\addplot [blue, thick] {x^0.5} node [pos=0.85, above] {$f(x)=\sqrt{x}$};
				...
				plot coordinates {
					(0,0) (1,1) (2,1.414214) (3,1.732051) (4,2) (5,2.236068)
				};
	...
	Agora, imagine fazer isso para milhares de retângulos. Precisamos, então, de uma maneira mais curta de escrever essa ideia matemática, e que sirva para calcularmos para quaisquer intervalos. Para isso, usamos a notação de somatório, representado pela letra sigma maiúscula $\sum$.
	...
	\begin{condicoes}
		\sum & Somatório\\
		(x_{i+1} - x_i) \cdot f(x_i) & Expressão de referência\\
		i & Valor mais baixo\\
		n & Valor mais alto
	\end{condicoes}
	...
	\begin{align*}
		A_t &= \sum_{i=1}^5 (x_{i+1} - x_i) \cdot f(x_i)\\
		A_t &= [(x_2 - x_1)f(x_1)] + [(x_3 - x_2)f(x_2)] + [(x_4 - x_3)f(x_3)] + [(x_5 - x_4)f(x_4)] + [(x_6 - x_5)f(x_5)]\\
		A_t &= [(2-1)f(1)] + [(3 - 2)f(2)] + [(4 - 3)f(3)] + [(5 - 4)f(4)] + [(6 - 5)f(5)]\\
		A_t &= [1 \cdot \sqrt{1}] + [1 \cdot \sqrt{2}] + [1 \cdot \sqrt{3}] + [1 \cdot \sqrt{4}] + [1 \cdot \sqrt{5}]\\
		A_t &\approxeq 8,4
	\end{align*}
	...
	\begin{center}
		\begin{tikzpicture}
			\begin{axis}[
				...
				plot coordinates {
					(0.5, 0.707) (1,1) (1.5,1.22) (2,1.41) (2.5,1.58) (3,1.73) (3.5,1.87) (4,2) (4.5,2.12) (5,2.24)
				};
	...
	Agora que conhecemos o truque, considere que a base é muito pequena, tendendo a zero. Denotamos, também, a diferença $x_i - x_0$ por $\Delta x_i$. A aproximação será a melhor possível quando a maior variação em $x$ tender a zero, ou seja, quando $\max{\Delta x_i} \to 0$:
	
	\begin{equation}\label{eq:integral_riemann}
		\boxed{A_t = \lim_{\max{\Delta x_i} \to 0} \,\sum_{i=1}^{n} f(x_i) \Delta x_i}
	\end{equation}
	
	Chamamos a Equação~\ref{eq:integral_riemann} acima de \textit{Integral de Riemann}, e outra forma de indicá-la é:
	...
	\begin{condicoes}
		\int & símbolo que representa a soma\\
		a, b & extremos do intervalo\\
		f(x) & a função, que determina a altura do retângulo\\
		dx & indica a variação infinitesimal em $x$
	\end{condicoes}
	
	Observe como os intervalos foram adicionados na integral como $\displaystyle \int_{a}^{b}$. Dizemos, então, que essa integral é definida.
	
	Analogamente, dizemos a integral é indefinida quando omitimos os extremos, como em $\displaystyle \int f(x)\,dx$.
	
	\begin{snugshade}
		\textbf{\textcolor{violet}{Você sabia?}}
		
		O matemático Gottfried Wilhelm Leibniz (1646 - 1716) foi o responsável por introduzir o famoso símbolo $\int$ para indicar a soma infinita de números infinitesimalmente pequenos e tem origem na letra antiga \char"017F (S longo).
	\end{snugshade}
	...
	\subsubsection{Primitivas}
		...
		\begin{definition}
			Seja $f$ uma função definida em um intervalo. Dizemos que $F$ é uma função primitiva definida nesse intervalo de $f$ quando $F'$ for exatamente igual a $f$.
		\end{definition}
		
		
		\begin{example}
			Seja $f(x) = x^2$. Determine uma primitiva de $f$.
		...
		\begin{snugshade}
			...
			Uma função contínua tem infinitas primitivas. Chamamos o conjunto de todas as primitivas de uma função por \textit{integral indefinida}.
		...
		\begin{table}[h!]
			...
			\begin{tabular}{|>{\centering\arraybackslash}m{6cm}|>{\centering\arraybackslash}m{8cm}|}
				...
				\hline
				$\displaystyle \int x^\alpha \, dx$ & $\displaystyle \dfrac{x^{\alpha + 1}}{\alpha + 1} + k$, $\alpha \neq -1$ \\
				\hline
	...
	\subsubsection{O Teorema Fundamental do Cálculo (TFC)}
	
	\begin{theorem}[Teorema Fundamental do Cálculo]
		Se uma função $f$ for integrável em um intervalo $[a,b]$, e se $F$ for uma primitiva de $f$ no intervalo $[a.b]$, então
		\begin{equation}
			\boxed{\int_a^b f(x) \,dx = F(b) - F(a)}
		\end{equation}
		
		Indicamos a diferença $F(b) - F(a)$ por $[F(x)]\,|_a^b$.
	\end{theorem}
	
	\begin{example}
		Seja $f(x) = x$, onde $x \in [0,1]$. Calcule a área abaixo da curva de $f(x)$.
		
		\begin{align*}
			\int_0^1 x \, dx &= \left[ \dfrac{x^2}{2} \right]_0^1 \quad \text{(Substituindo os extremos)}\\
		...
		\end{align*}
			...
			\begin{center}
				...
				\begin{axis}[
					...
					samples=100,
					domain=0:1,
					fill between/on layer={axis grid},
					]
					
					\addplot [name path=f, thick, blue] {x};
					\addplot [name path=axis, draw=none] {0};
					
					\addplot [gray!50] fill between [of=f and axis, soft clip={domain=0:1}];
					
					\draw [dashed] (1,0) -- (1,1) -- (0,1);
					
					\node at (axis cs:0.8,0.12) [anchor=south east] {$\displaystyle \int_0^1 x \, dx = \dfrac{1}{2}$};
	...
	\textbf{E quando a curva está abaixo do eixo $x$?}
	
	A integral irá representar uma área quando a função for sempre positiva.
	
	\begin{snugshade}
		\textbf{\textcolor{blue}{Relembre!}} Módulo: p.~\pageref{subsec:modulo}.
	\end{snugshade}
	
	\begin{example}
		Seja $f(x) = x$, onde $x \in [-1,0]$. Calcule a área abaixo da curva de $f(x)$.
		
		\begin{align*}
			\int_{-1}^{0} x \, dx &= \left[ \dfrac{x^2}{2} \right]_{-1}^{0} \quad \text{(Substituindo os extremos)}\\
			&= \dfrac{0^2}{2} - \dfrac{(-1)^2}{2} \\
			&= 0 - \dfrac{1}{2} \\
			&= -\dfrac{1}{2} \quad \text{(Como é possível obter a área de um valor negativo?)}
		\end{align*}
		
		O que queremos, então, é o valor absoluto da integral. Em outras palavras, $\left|-\dfrac{1}{2}\right|$.
		...
		\begin{center}
			\begin{tikzpicture}
				\begin{axis}[
					...
					\addplot [gray!50] fill between [of=f and axis, soft clip={domain=-1:0}];
					
					\draw [dashed] (-1,0) -- (-1,-1) -- (0,-1);
					
					\node at (axis cs:-0.7,-0.2) [anchor=north east] {$\dfrac{1}{2}$};
	...
	\subsubsection{Propriedades das integrais}
	...
	\subsubsection{Técnica de substituição}
		
		\begin{theorem}[Técnica de Substituição]
		...
		\begin{example} Calcule $\displaystyle \int \cos{2x} \, dx$ usando a técnica de substituição.
		...
		A chave para a substituição é compreender onde há problema na integração. Veja, sabemos integrar $\cos{x}$, mas não sabemos integrar $\cos{2x}$.
			...
			\begin{align*}
				\text{(Substituindo)} \quad u &= 2x \\
				\text{(A diferencial será) } \quad \dfrac{du}{dx} &= 2 \\
				du &= 2 \, dx \implies dx = \dfrac{1}{2} \, du
			\end{align*}
			
			Substituindo na integral original:
			$$
			\int \cos{2x} \, dx = \int \cos{u} \cdot \dfrac{1}{2} \, du = \dfrac{1}{2} \int \cos{u} \, du
			$$
			
			Integrando em relação a $u$:
			$$
			\dfrac{1}{2} \int \cos{u} \, du = \dfrac{1}{2} \sin{u} + K, \quad K \in \mathbb{R}
			$$
			
			Substituindo $u$ de volta por $2x$:
			$$
			\int \cos{2x} \, dx = \dfrac{1}{2} \sin{2x} + K, \quad K \in \mathbb{R}
			$$
	...
	\textbf{Exercício 1. Calcule as integrais definidas.}
	
	\begin{itemize}
		\item[1)] \( \displaystyle \int_{0}^{1} (x + 3) \, dx \\
		= \left[ \dfrac{x^2}{2} + 3x \right]_0^1
		= \left( \dfrac{1}{2} + 3 \cdot 1 \right) - \left( 0 + 0 \right)
		= \dfrac{7}{2} \)
		
		\item[2)] \( \displaystyle \int_{-1}^{1} (2x + 1) \, dx \\
		= \left[ x^2 + x \right]_{-1}^1
		= (1 + 1) - (1 - 1)
		= 2 \)
		
		\item[3)] \( \displaystyle \int_{0}^{4} \dfrac{1}{2} \, dx \\
		= \left[ \dfrac{x}{2} \right]_0^4
		= \dfrac{4}{2} - 0
		= 2 \)
		
		\item[4)] \( \displaystyle \int_{-2}^{1} (x^2 - 1) \, dx \\
		= \left[ \dfrac{x^3}{3} - x \right]_{-2}^1
		= \left( \dfrac{1}{3} - 1 \right) - \left( -\dfrac{8}{3} + 2 \right)
		= \left( -\dfrac{2}{3} \right) - \left( -\dfrac{2}{3} \right)
		= 0 \)
		
		\item[5)] \( \displaystyle \int_{1}^{3} dx \\
		= \left[ x \right]_1^3
		= 3 - 1
		= 2 \)
		
		\item[9)] \( \displaystyle \int_{0}^{2} (x^2 + 3x - 3) \, dx \\
		= \left[ \dfrac{x^3}{3} + \dfrac{3x^2}{2} - 3x \right]_0^2
		= \left( \dfrac{8}{3} + 6 - 6 \right) - 0
		= \dfrac{8}{3} \)
		
		\item[13)] \( \displaystyle \int_{-2}^{-1} \left( \dfrac{1}{x^2} + x \right) dx \\
		= \left[ -\dfrac{1}{x} + \dfrac{x^2}{2} \right]_{-2}^{-1}
		= \left( -\dfrac{1}{-1} + \dfrac{(-1)^2}{2} \right) - \left( -\dfrac{1}{-2} + \dfrac{(-2)^2}{2} \right) \\
		= \left( 1 + \dfrac{1}{2} \right) - \left( \dfrac{1}{2} + 2 \right)
		= \dfrac{3}{2} - \dfrac{5}{2}
		= -1 \)
		
		\item[14)] \( \displaystyle \int_{0}^{4} \sqrt{x} \, dx \\
		= \left[ \dfrac{2}{3}x^{3/2} \right]_0^4
		= \dfrac{2}{3}(4)^{3/2} - \dfrac{2}{3}(0)^{3/2} \\
		= \dfrac{2}{3} \cdot 8 - 0
		= \dfrac{16}{3} \) 
	\end{itemize}
	...
	\subsubsection{Integração por partes}
	...
	\begin{snugshade}
		...
		Recorde-se também que \textit{o que a derivada faz, a integral desfaz}.
	\end{snugshade}
	
	
	
	\begin{proof}[Integração por partes]
		
		Sejam $u(x)$ e $v(x)$ duas funções deriváveis. Então,
		\begin{align*}
			[u(x)v(x)]' &= u'(x)v(x) + u(x)v'(x) \quad \text{(Regra do Produto)}\\
			\int [u(x)v(x)]' \, dx &= \int u'(x)v(x) \, dx + \int u(x)v'(x) \, dx \\
			{\textcolor{red}{u(x)v(x)}} &= \int u'(x)v(x) \, dx + \int u(x)v'(x) \, dx \quad \text{\textcolor{red}{(Integral desfaz o que a derivada fez)}}\\
			{\textcolor{blue}{\int u(x)v'(x) \, dx}} &= u(x)v(x) - \int v(x)u'(x) \, dx \quad \text{\textcolor{blue}{(Isole a integral que sabemos resolver)}} \qedhere
		\end{align*}
	\end{proof}
	...
	\begin{example} Calcule $\displaystyle \int x \cdot \cos x \, dx$ utilizando a técnica de integração por partes.
		
		Sabemos que
		\begin{align*}
			\int x \cdot \cos x \, dx = u v - \int v \, du \\
		\end{align*}
	...
	\begin{example} Calcule $\displaystyle \int x e^x \, dx$, utilizando a regra de integração por partes.
		Escolhemos 
		$$
		u = x \quad \Rightarrow \quad du = dx
		$$
	...
	\begin{example}Calcule $\displaystyle \int \ln{x} \, dx$ utilizando a técnica de integração por partes.
		Escolhemos 
		$$
	...
	A seguir, veremos algumas aplicações das integrais nas ciências da natureza e na matemática pura.
		...
		Sejam \( a \) um número real diferente de zero e a função \( a(t) = a \) uma função que, para cada instante \( t \geq 0 \), nos retorna a aceleração de uma partícula que desloca-se em linha reta (em uma dimensão), em \( \text{m/s}^2 \), ao longo do tempo.
		
		\begin{enumerate}
			\item[(a)] Considere dois instantes de tempo \( t_0 \) e \( t \), tais que \( t_0 \) seja um valor conhecido e \( t \) desconhecido. Calcule a integral definida da função \( a(t) \) no intervalo de \( t_0 \) a \( t \). (Sugestão: Para não confundir o \( t \) do intervalo com o \( t \) do argumento da função \( a(t) \), denote o argumento por \( T \), isto é, \( a(T) \).)
			
			\item[(b)] No resultado do item anterior, há um termo constante que não depende do valor desconhecido \( t \), denote-o por \( v_0 \); a expressão toda por \( v(t) \). Explique o que esta expressão significa fisicamente e qual sua unidade.
			
			\item[(c)] Considere dois instantes de tempo \( t_0 \) e \( t \), tais que \( t_0 \) seja um valor conhecido e \( t \) desconhecido. Calcule a integral definida da função \( v(t) \) no intervalo de \( t_0 \) a \( t \). (Sugestão: Para não confundir o \( t \) do intervalo com o \( t \) do argumento da função \( v(t) \), denote o argumento por \( T \), isto é, \( v(T) \).)
			
			\item[(d)] No resultado do item anterior, há um termo constante que não depende do valor desconhecido \( t \), denote-o por \( S_0 \); a expressão toda por \( S(t) \). Explique o que esta expressão significa fisicamente e qual sua unidade.
			
			\item[(e)] Supondo que \( a = g = 9{,}8 \, \text{m/s}^2 \) e que esta aceleração é contra o referencial de baixo para cima, escreva as equações anteriores fazendo as devidas adaptações. Qual movimento estas equações descrevem?
		\end{enumerate}
		
		\textbf{Solução do Exercício 1}
		
		\begin{enumerate}
			\item[(a)] Devemos calcular a integral
			...
			\item[(b)] Fazendo \( v_0 = a t_0 \), temos que a expressão será
			...
			Como a unidade da constante \( a \) é \( \text{m/s}^2 \), ao multiplicar por \( t \) (que possui unidade \( \text{s} \)), temos \( \text{m/s} \). Esta nova unidade representa a velocidade e, portanto, ao integrar a função aceleração, obtém-se a função velocidade.
			
			\item[(c)] Devemos calcular a integral
			
			
			\[
			\int_{t_0}^{t} v(T)\,dT = \int_{t_0}^{t} (aT + v_0)\,dT = \left[\dfrac{a T^2}{2} + v_0 T\right]_{t_0}^{t}
			\]
			...
			\[
			S(t) = \dfrac{a t^2}{2} + v_0 t - \left(\dfrac{a t_0^2}{2} + v_0 t_0\right)
			\]
			
			
			
			\item[(d)] Fazendo \( S_0 = \dfrac{a t_0^2}{2} + v_0 t_0 \), temos:
			
			
			\[
			S(t) = \dfrac{a t^2}{2} + v_0 t + S_0
			\]
			
			
			Como a unidade da função \( v(t) \) é \( \text{m/s} \), ao multiplicar por \( t \), encontramos \( \text{m} \). Esta nova unidade representa a posição e, portanto, ao integrar a função velocidade, tem-se a função posição.
			
			\item[(e)] Utilizando a ideia de referencial dada no enunciado, temos que:
			
			
			\[
			a(t) = 9{,}8; \quad v(t) = 9{,}8t + v_0; \quad S(t) = \dfrac{9{,}8 t^2}{2} + v_0 t + S_0
			\]
		...
		\textbf{Solução do Exercício 2}
		
		Para a aceleração, derivamos a função dada:
		
		
		\[
		v'(t) = a(t) \implies a(t) =- \sin{\left(2t - \dfrac{\pi}{6}\right)} \cdot [2-0] \implies a(t) = -2 \sin{\left(2t - \dfrac{\pi}{6}\right)}
		\]
		
		
		
		Para a posição, integramos a função dada:
		
		
		\[
		\int v(t) \,dt = S(t) \implies S(t) = \dfrac{1}{2} \sin{\left(2t - \dfrac{\pi}{6}\right)} + K, K \in \mathbb{R}
		\]
		...
		\begin{enumerate}
			\item[(a)] Determine em quais intervalos de tempo esta partícula possui velocidade negativa.
			
			\item[(b)] Determine em quais intervalos de tempo esta partícula possui aceleração positiva.
		\end{enumerate}
		
		\textbf{Solução do Exercício 3}
		
		\begin{enumerate}
			\item[(a)] Derivando a função dada:
			
			
			\[
			S'(t) = v(t) \implies v(t) = 3t^2 - 1 
			\]
			
			
			Se queremos \( v(t) < 0 \) em \(t \geq 0\), impomos:
			
			
			\[
			v(t) < 0  \implies 3t^2 - 1  <0  \implies - \dfrac{1}{\sqrt{3}} < t < \dfrac{1}{\sqrt{3}}
			\]
			
			
			
			\item[(b)] Derivando a função velocidade:
			
			
			\[
			v'(t) = a(t) \implies a(t) = 6t 
			\]
			
			
			Se queremos \( a(t) > 0 \) em \(t \geq 0\), impomos:
			
			
			\[
			a(t) > 0 \implies 6t > 0 \implies t>0
			\]
			
			
		\end{enumerate}
	
	\subsubsection{Sólidos de revolução} 
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			% plot 
			\addplot[blue, smooth, domain=1.57:4] {- ln(x)*cos(deg(x))+0.2};
			%\addplot[blue!50, smooth, domain=1.57:4] {ln(x)*cos(deg(x))-0.2};
		\end{axis}
	\end{tikzpicture}
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			% plot paramétrico do cone
			\addplot[blue, smooth, domain=1.57:4] {- ln(x)*cos(deg(x))+0.2};
			\addplot[green!50, smooth, domain=1.57:4] {ln(x)*cos(deg(x))-0.2};
			\node[below] at (axis cs:4,0) {\small $b$};
			\node[below] at (axis cs:0,0) {\small $a$};
			\draw[red] (axis cs:4,0) -- (axis cs:4,1.10) node[midway, left]{$R$};
		\end{axis}
	\end{tikzpicture}
	
	
	
	Na nossa figura, vemos que a base é igual a área da circunferência, cujo raio $R$ é a imagem da função $f$, e a altura é dada por $b-a$. Podemos reescrever o volume da seguinte forma:
	\begin{align*}
		V &= A_b \cdot h\\
		V &= \pi R^2 \cdot (b-a)\\
	...
	Podemos particionar o intervalo para que $\Delta x$ seja um número muito pequeno, assim como fizemos na aula 7 (P.~\pageref{subsec:integrais}). Logo, ao fazermos $\max \Delta x \to 0$, teremos
	\begin{align*}
		V &= \lim_{\max{\Delta x_i} \to 0} \sum_{i=1}^{n} \pi [f(x_i)]^2 \Delta x_i\\
		\Aboxed{V &= \int \pi [f(x)]^2 \, dx}
	\end{align*}
	...
	\subsubsection{Exercício 4}
		...
		Calcule o volume do cone abaixo indicado.
		
		\begin{figure}[h!]
			\centering
			% Gráfico 2D da função afim
			\begin{subfigure}[b]{0.45\textwidth}
				\begin{tikzpicture}
					\begin{axis}[
						...
						\addplot[green!50, thick] {-x};
						\node[anchor=south west, blue] at (axis cs:4,2.3) {\small $f$};
					\end{axis}
				\end{tikzpicture}
				\caption{Função afim \( f(x) = \dfrac{R}{h}x \)}
			\end{subfigure}
			\hfill
			% Gráfico 3D do cone gerado pela rotação
			\begin{subfigure}[b]{0.45\textwidth}
				\begin{tikzpicture}
					\begin{axis}[
						view={15}{30},
						...
						%({x},{x*sin(deg(y))},{x*cos(deg(y))}); % Descobri que o cone fica voltado para cima. Mas eu quero que cresça em torno de z, proporcionalmente a x
						({x}, {x*sin(deg(y))}, {x*cos(deg(y))});
					\end{axis}
				\end{tikzpicture}
				\caption{Cone gerado pela rotação da função}
			\end{subfigure}
			\caption{Ilustração da função afim e do sólido gerado}
			\end{figure}
			
		\textbf{Solução do Exercício 4}
		
		A função da Fig. é afim. Ou seja, podemos escrevê-la na forma $f(x) = ax + b$, onde $a$ é o coeficiente angular e $b=0$ é o termo independente.
		...
		\begin{align*}
			...
			V &= \int_{0}^{h} \pi \left[\dfrac{R}{h}x\right]^2 \, dx\\
			V &= \int_{0}^{h} \pi \dfrac{R^2}{h^2}x^2 \, dx \quad \text{( $\dfrac{\pi R^2}{h^2}$ é constante)}\\
			V &= \dfrac{\pi R^2}{h^2} \int_{0}^{h} x^2 \, dx\\
		\end{align*}
		
		Assim, encontramos que
		\begin{align*}
			V &= \dfrac{\pi R^2}{h^2} \int_{0}^{h} x^2 \, dx\\
			&= \dfrac{\pi R^2}{h^2} \left[\dfrac{x^3}{3}\right]\bigg|_0^h\\
			&= \dfrac{\pi R^2}{h^2} \left[\dfrac{h^3}{3}\right] - \left[\dfrac{0^3}{3}\right]\\
			&= \dfrac{\pi R^2}{\cancel{h^2}} \left[\dfrac{\cancel{h^3}}{3}\right]\\
			&= \dfrac{\pi R^2 h}{3}
		\end{align*}
	
	\subsubsection{Exercício 5}
	
	Calcule o volume da esfera abaixo indicada.
		\begin{snugshade}
			\textbf{\textcolor{blue}{Relembre!}}
		...
		\begin{figure}
			...
			\begin{subfigure}[b]{0.45\textwidth}
				\begin{tikzpicture}
					\begin{axis}[
						...
						\addplot[blue, thick] {sqrt(4 - x^2)};
						\addplot[green!50, thick] {- sqrt(4 - x^2)};
					\end{axis}
				\end{tikzpicture}
				\caption{Função $f(x) = \sqrt{r^2 -x^2}$ e a rotação em torno do eixo $x$}
			\end{subfigure}
			\hfill
			\begin{subfigure}[b]{0.45\textwidth}
				\begin{tikzpicture}
			...
			\end{subfigure}
			\caption{Ilustração da função e da esfera}
		\end{figure}
		
		\textbf{Solução do Exercício 5}
		
		Observe, contudo, que a equação da circunferência não é uma função (veja P.~\pageref{proof:circunferencia}). Assim, calculamos a área no intervalo $[-r,r]$, ou seja, o semicírculo.
		
		Agora, a equação da circunferência cujo centro é $(0,0)$ pode ser escrita em função de $x$:
		\begin{align*}
			r^2 &= (x-x_c)^2 + (y - y_c)^2\\
			r^2 &= (x-0)^2 + (y - 0)^2 \quad \text{(Substituímos $x_c=0$ e $y_c=0$)}\\
			r^2 - x^2 &= y^2\\
			\sqrt{r^2 - x^2} &= y\\
			y &= \sqrt{r^2 - x^2} \quad \text{(Reorganizando a expressão)}
		\end{align*}
		...
		\begin{align*}
			V &= \pi \left[ r^2x - \dfrac{x^3}{3} \right]\bigg|_{-r}^{r} \\
			V &= \pi \left( r^3 - \dfrac{r^3}{3} \right) - \pi \left( -r^3 + \dfrac{(-r)^3}{3} \right) \\
			V &= \pi \left( r^3 - \dfrac{r^3}{3} + r^3 - \left( -\dfrac{r^3}{3} \right) \right) \quad \text{($\pi$ em evidência)}\\
			V &= \pi \left( 2r^3 - \dfrac{2r^3}{3} \right) \\
			V &= \pi \left( \dfrac{6r^3 - 2r^3}{3} \right) \\
			V &= \dfrac{4\pi r^3}{3}
		\end{align*}
		
	
	\subsubsection{Exercício 6}
	Considere a função $f(x) = \dfrac{1}{x}$, cujo gráfico é representado abaixo. Calcule o volume gerado pela figura ao rotacionar a função em torno do eixo $x$ no intervalo $[1,10]$.
			...
			\begin{subfigure}[b]{0.45\textwidth}
				\begin{tikzpicture}
					\begin{axis}[
						...
						\addplot[blue!50, thick] {-1/x};
						\node[anchor=south west, blue] at (axis cs:4,2.3) {\small $f$};
					\end{axis}
				\end{tikzpicture}
				\caption{Função \( f(x) = \dfrac{1}{x} \)}
			\end{subfigure}
			\hfill
			% Gráfico 3D do cone gerado pela rotação
			\begin{subfigure}[b]{0.45\textwidth}
				\begin{tikzpicture}
					...
					\end{axis}
				\end{tikzpicture}
				\caption{Figura gerada pela rotação da função}
			\end{subfigure}
			\caption{Ilustração da função $\dfrac{1}{x}$ e a rotação em torno de $x$}
		\end{figure}
		
		\textbf{Solução do Exercício 6}
		
		Observe que $D_f = ]-\infty, 0] \cup [0, \infty+[ \text{ , e} \, Im_f= ]-\infty, 0] \cup [0, \infty+[\,$. Escolhemos analisá-la no intervalo $[1,10]$. Novamente, rotacionamos a função em torno do eixo $x$. A equação do volume pode ser escrita como
		\begin{align*}
			V &= \pi \left(\dfrac{1}{x}\right)^2 \Delta x\\
			V &= \pi \dfrac{1}{x^2} \Delta x \quad \text{(Note: $x \neq 0$)}
		\end{align*}
		
		Integrando, temos 
		
		\begin{align*}
			V &= \pi \int_{1}^{10} \dfrac{1}{x^2} \, dx\\
			V &=  \pi \left[\dfrac{x^{-1}}{-1}\right]\bigg|_1^{10}\\
			V &=  \pi \left[-\dfrac{1}{x}\right]\bigg|_1^{10}\\
			V &= \pi \left[- \dfrac{1}{10} - \left(- \dfrac{1}{1}\right)\right]\\
			V &=  \dfrac{9}{10}\pi
		\end{align*}
	...
	\begin{align*}
		V &= \pi \int_{1}^{1000} \dfrac{1}{x^2} \, dx\\
		V &= \pi \left[- \dfrac{1}{1000} - \left(- \dfrac{1}{1}\right)\right]\\
		V &=  \dfrac{999}{1000}\pi
	\end{align*}
	
	A área está se aproximando cada vez mais de $\pi$ à medida que o extremo superior do intervalo aumenta.
	
	\subsection{Integrais impróprias}
	
	Até o momento, integrávamos em intervalos limitados e fechados. Contudo, se escolhermos um intervalo $[1,c]$, de modo que $c \to \infty$, teremos
	
	\begin{align*}
		\lim_{c \to \infty+} V &= \lim_{c \to \infty+}  \int_{1}^{c} \pi \dfrac{1}{x^2} \, dx\\
		\lim_{c \to \infty+} V &= \pi \left[- \dfrac{1}{c} - \left(- \dfrac{1}{1}\right)\right]\\
		\lim_{c \to \infty+} V &= \pi \left[0 - \left(- \dfrac{1}{1}\right)\right]\\
		\lim_{c \to \infty+} V &= \pi 
	\end{align*}
	
	Observe que $- \dfrac{1}{c}$ indica um número ($1$) sendo dividido por um número infinitamente grande ($c$). Logo, o resultado dessa divisão tende a zero. Logo, o limite do volume da figura é $\pi$.
	
	Chamamos essa integral de \textit{integral imprópria} com limite no infinito positivo, e como o limite existe, dizemos que essa integral é convergente.
