...
\renewcommand{\arraystretch}{1}
	...
	\subsection{Resolvendo exercícios}
	%% Resolver: escrever limite em l, m, n, o (tem que resolver com as raízes), q, t, 
	\begin{Ex}Calcule e justifique.\end{Ex}
	
	\begin{sol}$(a)$ Como a função $f(x) = x^{2}$ é contínua, temos que \begin{center}$\displaystyle{\lim_{x \to 2} x^{2}} = f(2) = 2^{2} = 4 \Longrightarrow \displaystyle{\lim_{x \to 2} x^{2}} = 4$\end{center}
	...
	\begin{table}[h]
		...
		\begin{tabular}{|>{\centering\arraybackslash}m{3cm}|>{\centering\arraybackslash}m{10cm}|}
			...
			\hline
			Quociente & $\displaystyle \left[\dfrac{f(x)}{g(x)}\right]' = \dfrac{f'(x)g(x) - f(x)g'(x)}{[g(x)]^2}$ \\
			\hline
			Cadeia & $\displaystyle [f(g(x))]' = f'(g(x)) \cdot g'(x)$\\
			\hline
		\end{tabular}
	\end{table}
	\renewcommand{\arraystretch}{1.0}
	...
	Derivadas são ferramentas úteis para analisar o comportamento de funções ou esboçar seu gráfico. Vejamos o exemplo a seguir:
		...
		\begin{example}
			...
			\textit{Passo 5:} Podemos confirmar com o teste da segunda derivada da função:
		...
		\begin{table}[h]
			...
			\begin{tabular}{|>{\centering\arraybackslash}m{6cm}|>{\centering\arraybackslash}m{8cm}|}
				\hline
				$\displaystyle f'(x) = 0$ & Candidatos a pontos críticos \\
				\hline
				$\displaystyle f'(x) > 0$ & A função cresce \\
				\hline
				$\displaystyle f'(x) < 0$ & A função decresce \\
				\hline
				$\displaystyle f''(x) > 0$ & Ponto de mínimo \\
				\hline
				$\displaystyle f''(x) < 0$ & Ponto de máximo \\
				\hline
			\end{tabular}
		\end{table}
	...
	\begin{itemize}
		...
		\item[f)] \( g(t) = \ln(2t + 1) \)
		\item[g)] \( x = e^{\sin t} \)
		\item[i)] \( y = (\sin x + \cos x)^3 \)
		\item[j)] \( y = \sqrt{3x + 1} \)
		\item[l)] \( f(x) = \sqrt[3]{\dfrac{x - 1}{\sqrt{x} + 1}} \)
		\item[n)] \( f(x) = \ln(t^2 + 3t + 9) \)
		\item[p)] \( y = \sin(\cos x) \)
	\end{itemize}
	...
	\begin{itemize}
		...
		\item[c)] \( f'(x) = 3e^{3x} \)
		\item[d)] \( f'(t) = -8\sin(8t) \)
		\item[e)] \( y' = 3t^2\cos(t^3) \)
		\item[f)] \( g'(t) = \dfrac{2}{2t + 1} \)
		\item[g)] \( f'(x) = e^{\sin t} \cdot (\sin t)' = e^{\sin t} \cdot \cos t \)
		\item[i)] 
		
		
		\[
		y' = 3(\sin x + \cos x)^2 \cdot (\cos x - \sin x)
		\]
		...
		\[
		y' = (\sqrt{3x + 1})' = \left[(3x+1)^\dfrac{1}{2}\right]' \cdot (3x+1)'
		= \dfrac{3}{2\sqrt{3x + 1}}
		\]
		...
		\[
		f'(x) = \dfrac{1}{3} \cdot \left(\dfrac{x-1}{x+1}\right)^{\dfrac{-2}{3}} \cdot \dfrac{2x}{(x+1)^2}
		= \dfrac{2x}{3(x+1)^2} \cdot \sqrt[3]{\left(\dfrac{x+1}{x-1}\right)^2}
		\]
		...
		\[
		f'(t) = \dfrac{2t + 3}{t^2 + 3t + 9}
		\]
		
		\item[p)] \( y' = -\cos(\cos x)\cdot\sin x \)
